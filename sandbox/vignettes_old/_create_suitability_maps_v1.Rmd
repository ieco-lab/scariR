...
regional_ensemble_1995_plot <- ggplot() +
  ...
  scale_fill_gradientn(
    ...
    values = c(
      0, # minimum
      as.numeric(regional_ensemble_thresholds[1, 4]), # MTP thresh
      as.numeric(regional_ensemble_thresholds[1, 4]) + 0.0000001, # some small amount more than the MTP
      as.numeric(regional_ensemble_thresholds[3, 4]), # MTSS thresh
      as.numeric(regional_ensemble_thresholds[3, 4]) + 0.0000001,
      # the rest
      0.5,
      0.75,
      1
    ...
    # the tick marks
    breaks = c(0, as.numeric(regional_ensemble_thresholds[1, 4]), as.numeric(regional_ensemble_thresholds[3, 4]), 0.5, 0.75, 1.00),
    # labels for the tick marks
    labels = c("unsuitable", "", paste0(round(as.numeric(regional_ensemble_thresholds[3, 4]), 2), "\n(MTSS thresh)"), 0.5, 0.75, 1),
    limits = c(0, 1.0),
...
regional_ensemble_2055_plot <- ggplot() +
  ...
  scale_fill_gradientn(
    ...
    values = c(
      0, # minimum
      as.numeric(regional_ensemble_thresholds[2, 4]), # MTP thresh
      as.numeric(regional_ensemble_thresholds[2, 4]) + 0.0000001, # some small amount more than the MTP
      as.numeric(regional_ensemble_thresholds[4, 4]), # MTSS thresh
      as.numeric(regional_ensemble_thresholds[4, 4]) + 0.0000001,
      # the rest
      0.5,
      0.75,
      1
    ...
    colors = c("azure4", "azure4", "azure2", "azure2", "#481567FF", "#2D708EFF", "#3CBB75FF", "#FDE725FF"),
    # the tick marks
    breaks = c(0, as.numeric(regional_ensemble_thresholds[2, 3]), as.numeric(regional_ensemble_thresholds[4, 4]), 0.5, 0.75, 1.00),
    # labels for the tick marks
    labels = c("unsuitable", "", paste0(round(as.numeric(regional_ensemble_thresholds[4, 4]), 2), "\n(MTSS thresh)"), 0.5, 0.75, 1),
    limits = c(0, 1.0),
    guide = guide_colorbar(frame.colour = "black", ticks.colour = "black", barwidth = 20, draw.ulim = TRUE, draw.llim = TRUE)
    ) +
  # labs
