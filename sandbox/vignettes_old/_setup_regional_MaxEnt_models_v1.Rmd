...
Now, I will create a raster of buffers around these presence points. I will first rasterize the SLF presence records and then use that raster to calculate a 355km buffer. This raster will then be masked by atc_2015 (`mask_layer`) to cut out bodies of water. Lastly, I had to do some acrobatics to crop the resulting raster to the extent of the area that is NA only. I converted this raster to a data frame and dropped the NA values, then took the extent of that raster and used it to crop the final raster.
...
# need to take the extent of just the non NA areas. 
# I converted to a df and dropped non NA values. Then I converted back to a raster and took the extent of these points
non_na_extent <- terra::as.data.frame(buffered_invaded, xy = TRUE, na.rm = TRUE) %>%
  # convert to raster
  terra::rast(., crs = "EPSG:4326") 
# take the extent of this raster
non_na_extent <- terra::ext(non_na_extent)


# use the above extent to crop the raster
buffered_invaded <- terra::crop(x = buffered_invaded, y = non_na_extent)
...
writeRaster(x = buffered_invaded, filename = file.path(mypath, "models", "working_dir", "regional_invaded_presences_buffered_mask.tif"), filetype = "GTiff", overwrite = FALSE)

```

### trim bioclim layers to mask

Now that I have created a mask layer of the desired 355km buffer around the SLF presences, I will use this raster to both trim the bioclim layers for training the regional_invaded model and select the background points used to sample those layers. First, I will import the bioclim layers.
...
I will need to crop the main rasters by the smaller raster and then mask by the same raster, so the result is simply a raster of that size and shape. The output rasters also need to be cropped and converted to the .ascii format for MaxEnt.
...
if(TRUE) {
  ...
  for(a in seq_along(env.files)){
    ...
    rast.hold <- terra::crop(x = rast.hold, y = buffered_invaded)
    # mask by same raster to cut out bodies of water
...
bio15 <- terra::rast(x = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km", "bio15_1981-2010_invaded_buffered.asc")) 
# convert to df
bio15_df <- terra::as.data.frame(bio15, xy = TRUE)

# plot main layer as example
(ggplot() +
  # change scale of plots to be standard across figures
  geom_raster(data = bio15_df, 
            aes(x = x, y = y, fill = `CHELSA_bio15_1981-2010_V.2.1`)) +
  xlab("longitude") +
  ylab("latitude") +
  theme_minimal() +
  theme(legend.position = "none") +
...
mask_layer <- raster::raster(x = file.path(mypath, "models", "working_dir", "regional_invaded_presences_buffered_mask.tif"))
...
# crop and mask by weighted layer
global_mean_weighted <- global_mean_weighted %>%
  raster::crop(., y = mask_layer) %>%
  raster::mask(., mask = mask_layer) 
...
This function only selects 1 point per cell. With the other restrictions we put on the function, it actually could not select the desired 10,000 points. I decided to settle for 7,500 because it is a neat proportion of the desired total. This is also ok because this background area is far smaller than either the native or global model extents.
...
# convert mask layer to df
mask_layer_df <- as.data.frame(mask_layer, xy = TRUE, na.rm = TRUE)
...
(regional_points_plot <- ggplot() +
   ...
   geom_raster(data = mask_layer_df, aes(x = x, y = y), fill = "azure1") +
   geom_point(data = regional_points, aes(x = x, y = y), color = "darkorange", size = 0.001) +
...
I need to select points for China as well, but I do not need to weight these. Weighting was done to account for the lack of time for SLF to reach all potentially suitable habitats. The background selection here will be random, but will still exclude cells with SLF points and will correct for latitudinal stretching.
