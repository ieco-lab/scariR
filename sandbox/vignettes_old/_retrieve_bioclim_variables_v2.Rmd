---
title: "Retrieve bioclim rasters from CHELSA and tidy them for MaxEnt modeling (chapter 1)"
author: "Sam Owens"
date: "2024-01-05"
...
My first approach to downloading these data was to create a loop to open each URL in a web browser (be ready, as it literally just opens whatever URLs are selected from the list in a browser). Later, I will functionalize this so that the download can be more flexible. 
...
The EPSG number that matches the previous `proj4` string is `EPSG:4326`. We will use this notation from now on. 
...
Finally, we will convert the rasters to the `.ascii` format required by MaxEnt. 
...
if(FALSE){
  ...
  output.files <- list.files(path = file.path(mypath, "v1"), pattern = "\\.tif$", full.names = FALSE) %>%
    gsub(pattern = ".tif", replacement = "_10km.asc")
...
This additional step can be run on-demand. I will aggregate selected rasters to a lower resolution, with roughly 10km grid cells. We want to aggregate at 10km because we will be downsampling the resolution of our analysis to account for survey bias.

```{r on-demand aggregation of rasters}
...
if(FALSE){
  ...
  write.csv(x = env.cor, file = file.path(here::here(), "vignette-outputs", "data-tables", "env_cor_chelsa_downsampled.csv"), col.names = TRUE, row.names = TRUE)
...
env.cor2 <- read.csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "env_cor_chelsa_downsampled.csv")) 
...
colnames(env.cor3) <- gsub(pattern = "X2015_accessibility_to_cities_v1.0", replacement = "atc", x = colnames(env.cor3))
# simplify rownames
...
rownames(env.cor3) <- gsub(pattern = "2015_accessibility_to_cities_v1.0", replacement = "atc", x = rownames(env.cor3))

# factor levels to order row names
var.names <- c(x = "bio1", "bio2", "bio3", "bio4", "bio5", "bio6", "bio7", "bio8", "bio9", "bio10", "bio11", "bio12", "bio13", "bio14", "bio15", "bio16", "bio17", "bio18", "bio19", "gdd10", "gdgfgd10", "gddlgd10", "ngd10", "atc")

# reorder columns 
env.cor4 <- env.cor3 %>%
  dplyr::select(bio1, bio2:bio9, bio10:bio19, gdd10, gdgfgd10, gddlgd10, ngd10, atc) %>%
  # reorder rows
...
# write tidied data
write.csv(x = env.cor4, file = file.path(here::here(), "vignette-outputs", "data-tables", "env_cor_chelsa_downsampled_abs.csv"), col.names = TRUE, row.names = TRUE)
...
```{r prepare data to visualize}

env.cor5 <- read.csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "env_cor_chelsa_downsampled_abs.csv"))
...
var.renames <- c( 
  ...
  "gdd10" = "growing DD heat sum above 10°C",
  "gddlgd10" = "last growing DD above 10°C",
  "gdgfgd10" = "first growing DD above 10°C",
  "ngd10" = "# growing DD above 10°C"
)

```

```{r visualize correlations via heatmap}
...
I have found that choosing covariates can be rather ad-hoc, so I developed what I believe is the best-fit methodology for choosing covariates out of much practice. First, I look at the different groupings of covariates (those based on temperature, precipitation and degree-day) and choose a representative from each group. The ideal representative is highly correlated with most of the others in the group, which indicates its predictive power (SOURCE), and represents a generality of its group (ie, the mean yearly average). In this case, I chose mean temperature of the coldest quarter and precip (bio 11 and 12). Besides its correlation values, mean temp of the coldest quarter is highly impactful for the survival of SLF eggs (Lee et.al, 2011). 

According to this methodology, I might also have chosen access to cities (atc) because it is not correlated with any other variables. I will be using this variable in a later model of human impacts, but not in my historical and climate change models. This is because ATC is a static variable and I have no way to project it for climate change, while the bioclim variables come with climate change versions.

None of the GDD variables were chosen because they were all highly correlated with bio 11 or each other.
...
ggsave(env.cor_plot, filename = file.path(here::here(), "vignette-outputs", "figures", "env_cor_CHELSA_v1.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
```{r binarize correlation heatmap}
...
ggsave(env.cor_binary_plot, filename = file.path(here::here(), "vignette-outputs", "figures", "env_cor_CHELSA_v1_binary.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
The final 4 variables have been selected for SDM of Lycorma delicatula, which include "mean diurnal range", "mean temperature of the coldest quarter", "annual precipitation", and "precipitation seasonality". These covariates can be projected for climate change and are biologically relevant to our target species. In the next vignette, we will crop these covariates to the appropriate spatial scale to run regional models. We will also use these rasters to create background point sets, which MaxEnt uses to sample the environmental covariates. 



# Appendix

## Bioclim Layers for MaxEnt Java

These chunks are identical to chunks 8 and 11, except these chunks convert NA values to -9999 in the rasters created. This is the default value for NA taken by the Java version of Maxent, but this can also be changed in the app.
...
if(FALSE){
  ...
  for(a in seq_along(env.files)){
    ...
    # write output
    terra::writeRaster(x = rast.hold, filename = file.path(mypath, "v1_maxent_java", output.files[a]), filetype = "AAIGrid", overwrite = FALSE)
...
```{r conversion of .tif to .asc files CMIP6}
...
Lee, J.-S., I.-K. Kim, S.-H. Koh, S. J. Cho, S.-J. Jang, S.-H. Pyo, and W. I. Choi. 2011. Impact of minimum winter temperature on Lycorma delicatula (Hemiptera: Fulgoridae) egg mortality. Journal of Asia-Pacific Entomology 14:123–125.
