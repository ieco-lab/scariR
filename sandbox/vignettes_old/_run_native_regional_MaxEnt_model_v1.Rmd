...
a_regional_native_background_points <- read.csv(file.path(here(), "vignette-outputs", "data-tables", "regional_native_background_points_v0.csv"))
...
if (FALSE) {
  ...
  regional_native_train <- SDMtune::prepareSWD(species = "Lycorma delicatula",
...
SDMtune::swd2csv(swd = regional_native_train, file_name = c(
  file.path(here(), "vignette-outputs", "data-tables", "regional_native_train_slf_presences_with_data.csv"),
...
if (FALSE) {
  ...
  regional_native_test <- SDMtune::prepareSWD(species = "Lycorma delicatula",
...
SDMtune::swd2csv(swd = regional_native_test, file_name = c(
  file.path(here(), "vignette-outputs", "data-tables", "regional_native_test_slf_presences_with_data.csv")
  ))
...
if (FALSE) {
  ...
  getTunableArgs(regional_native_model)
  ...
  regional_native_model_tuned <- SDMtune::gridSearch(
  ...
  regional_native_model_tuned@results
  # choose model with highest test AUC
  best.model <- which.max(regional_native_model_tuned@results$test_AUC)
  
}

```

Based on the model tuning, we see that the highest test AUC value was attributed to the model with a regularization multiplier of 1.5 and hinge features (model 1). We will need to re-train the model using these parameters.
...
mypath <- file.path(here() %>% 
                       dirname(),
                     "maxent/models/slf_regional_native_v0")
...
# saved to here()/vignette-outputs/figures, "regional_native_model_tuning_results.jpg"

# save df results of model tuning
write.csv(regional_native_model_tuned@results, file = file.path(mypath, "regional_native_model_tuning_results.csv"))

# save model tuning object
write_rds(regional_native_model_tuned, file = file.path(mypath, "regional_native_models_tuned.rds"))
