...
edits by Sam Owens, 2022-03-03- Changed WD so that I can read in data, installed and libraried additional packages

questions to ask Nadege: 
- can you explain the concept of calculating pairwise distances between points and what you did on these lines? (lines 66-71)
- please explain for loops and if statements in lines 71-87
- I would like some tips on creating a custom function. How do I test a function as I am creating it? How would I begin to create a function? How do I know when I could make a process easier and save time by creating a function?
- for loop: repeating an action on every element of a list. Needs a separate list
- if statement- if (condition is met) {do action}
  - else if: adds additional "if" statement to another if statement
- while loop: same as a for loop but runs only until a condition is fulfilled
...
Jumps <- read.csv(
  file.path(here(), "data_raw", "(Nadege) jumps_raw.csv"), h=T
  )
