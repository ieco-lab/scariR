...
mypath <- file.path(here() %>% 
 ...
 env.files.2 <- env.files[env.files %in% c("C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/future_climate_rasters_CMIP6/2021-2040_ssp370_30arcsec_GISS/v1_CA/global_bio_1.tif",
                                          "C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/future_climate_rasters_CMIP6/2021-2040_ssp370_30arcsec_GISS/v1_CA/global_bio_2.tif",
                                          "C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/future_climate_rasters_CMIP6/2021-2040_ssp370_30arcsec_GISS/v1_CA/global_bio_7.tif",
                                          "C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/future_climate_rasters_CMIP6/2021-2040_ssp370_30arcsec_GISS/v1_CA/global_bio_12.tif",
                                          "C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/future_climate_rasters_CMIP6/2021-2040_ssp370_30arcsec_GISS/v1_CA/global_bio_15.tif"
                                          )]
...
# asc_to_tif <- raster(file.path("C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040/Lycorma_delicatula_avg.asc")) 
...
main_layer.2 <- crop(raster(file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040/Lycorma_delicatula_avg.asc")), 
                     y = current.extent, overwrite = F)

# writeRaster(x = asc_to_tif, filename = file.path("C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040/asc_to_tif/Lycorma_delicatula_avg"), format = "GTiff", overwrite = F)
...
writeRaster(x = current_tif, filename = file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040/manipulated_outputs/Lycorma_delicatula_avg_CA.tif"),
            format = "GTiff", 
            overwrite = F)
...
writeRaster(x = current_tif, filename = file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040/manipulated_outputs/Lycorma_delicatula_avg_CA.asc"),
            format = "ascii",
            overwrite = F)
...
current_CA <- raster(file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040/manipulated_outputs/Lycorma_delicatula_avg_CA.asc")) 
...
future_CA <- raster(file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040/Lycorma_delicatula_v1_maxent_CA_2021-2040_avg.asc")) 
...
# selecting only the files I need for my analysis to speed up code, as of 2022-11-12
env.files.4 <- env.files[env.files %in% "C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/historical_climate_rasters/originals/2015_accessibility_to_cities_v1.0.tif"]
...
mypath <- file.path(here() %>% 
  ...
  # subset of files needed for next model
  env.files.5 <- env.files[env.files %in% "C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/historical_climate_rasters/v1/global_atc_CA_current.tif"]
...
CA_atc <- raster(file.path(here() %>% 
                      dirname(),
                      "maxent/future_climate_rasters_CMIP6/2021-2040_ssp370_30arcsec_GISS/v1_maxent_CA_2021-2040/global_atc.asc")) 
...
# original output raster
#current_global <- raster(file.path("C:/Users/tun83449/OneDrive - Temple University/Shared drives/slfClimate/projects/slfSpread/maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040_withATC/Lycorma_delicatula_avg.asc")) 

# cropped version of current map
current_CA <- raster(file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040_withATC/manipulated_outputs/Lycorma_delicatula_avg_CA.asc")) 
...
future_CA <- raster(file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040_withATC/Lycorma_delicatula_v1_maxent_CA_2021-2040_avg.asc")) 

future_df <- as.data.frame(future_CA, xy = TRUE)

# cropped version of current map
current_napa <- raster(file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040_withATC/manipulated_outputs/Lycorma_delicatula_avg_CA.asc")) 
...
``` {r load in 2019 CA grape data}

# CA 2019 grape data
grapes <- raster(file.path(file.path(here() %>% 
                      dirname(),
                      "slfSpread/data/grapes_CA_2019_USDA.tif")) %>%
  # mask NA values
  # mask(., ., maskvalue = NA) %>%

# downsample raster resolution and write newly resized raster
grapes <- aggregate(grapes, fact = 2, fun = mean, expand = TRUE, na.rm = TRUE, filename = file.path(here() %>% 
                      dirname(),
                      "slfSpread/data/grapes_CA_2019_USDA_downsampled.tif"))
  
# convert to df while also excluding NA values
grapes_sf <- as.data.frame(grapes, xy = TRUE)[!is.na(as.data.frame(grapes, xy = TRUE)$grapes), ] 

coordinates(grapes_sf) <- c('x', 'y')
proj4string(grapes_sf) = CRS("+init=epsg:3310")
# transform CRS to WGS84
grapes_sf <- spTransform(grapes_sf, CRS("+init=epsg:4326")) 

grapes_df <- as.data.frame(grapes_sf)

# export as csv because it was awful to get this data frame
write_csv(grapes_df, file = file.path(here() %>% 
                      dirname(),
                      "maxentslfSpread/data/grapes_CA_2019_USDA.csv")

```

```{r pull max suitability value from current and future cropped CA rasters}

max_suitability <- data.frame(c(max(current_df$Lycorma_delicatula_avg_CA, na.rm = TRUE),
                                max(future_df$Lycorma_delicatula_v1_maxent_CA_2021.2040_avg, na.rm = TRUE))) %>%
  max(.)
...
current.extent <- extent(-124.409591, -114.131211, 32.534156, 42.009518)
# crop raster to extent
main_layer.3 <- crop(raster(file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040_withATC/Lycorma_delicatula_avg.asc")), 
                     y = current.extent, overwrite = F)
...
writeRaster(x = current_tif, filename = file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040_withATC/manipulated_outputs/Lycorma_delicatula_avg_CA.tif"), format = "GTiff", overwrite = F)

#also write as a .asc
writeRaster(x = current_tif, filename = file.path(here() %>% 
                      dirname(),
                      "maxent/maxent_models/maxent_slf_2022_gbif_CA_2021-2040_withATC/manipulated_outputs/Lycorma_delicatula_avg_CA.asc"), format = "ascii", overwrite = F)
...
(visualize_CA_current <- ggplot() +
  ...
  geom_raster(data = current_df, 
              aes(x = x, y = y, fill = Lycorma_delicatula_avg_CA)) +
  geom_polygon(data = map_data('state'), 
               aes(x = long, y = lat, group = group), 
               fill = NA, 
               color = "black",
               size = 0.75) +
  geom_point(data = as.data.frame(grapes_sf), aes(x = x, y = y), size = 0.1, color = "purple", alpha = 0.1) +
  # set xlim and ylim to CA bounding box
...
# export image
ggsave(filename = "Predicted Suitability for Establishment of Lycorma delicatula in CA, 1970-2000, with global_atc and grape_v0.jpg", 
       path = file.path(here(), "figures"),
...
visualize_CA_future <- ggplot() +
  ...
  geom_raster(data = future_df, 
              aes(x = x, y = y, fill = Lycorma_delicatula_v1_maxent_CA_2021.2040_avg)) +
  geom_polygon(data = map_data('state'), 
               aes(x = long, y = lat, group = group), 
               fill = NA, 
               color = "black",
               size = 0.75) +
  geom_point(data = as.data.frame(grapes_sf), aes(x = x, y = y), size = 0.1, color = "purple", alpha = 0.2) +
  # set xlim and ylim to CA bounding box
  coord_sf(xlim = c(-124.409591, -114.131211), 
           ylim = c(32.534156, 42.5), 
           expand = TRUE) +
...
ggsave(filename = "Predicted Suitability for Establishment of Lycorma delicatula in CA, 2021-2040, with global_atc and grape_v0.jpg", 
       path = file.path(here(), "figures"), 
...
# export image
ggsave(filename = "Predicted Suitability for Establishment of Lycorma delicatula in Napa county, CA, 1970-2000, with global_atc adn grape_v0.jpg",
...
# export image
ggsave(filename = "Predicted Suitability for Establishment of Lycorma delicatula in Napa county, CA, 2021-2040, with global_atc and grape_v0.jpg", 
