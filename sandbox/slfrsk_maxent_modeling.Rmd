...
=======
=======
>>>>>>> 8ffd63adb5b65d664d88f5c99f6fd25117dd8379
...
```{r overlay raster layers and resample to new thresholds}
>>>>>>> 8ffd63adb5b65d664d88f5c99f6fd25117dd8379

For my analysis, I need to compare the predictions of both the regional and global models in the invaded region. First, I will need to infer the amount of range filling by SLF according to each model. In Gallien et.al, this was done by plotting the points overtop of the raster of both the regional and global models and comparing the models for the level of agreement in suitability. There are a few steps to this process:

1. Convert each output to binary (suitable/unsuitable) based on a suitability threshold (likely the threshold rule chosen in MaxEnt, MTP), but could also be 0.5 "suitability value" bc Gallien et.al used this.
2. Overlay the regional and global model predictions
3. Sum rasters
...
if(FALSE) {
  ...
  # path to directory I will be working in
   ...
   # load in regional model
  v5_regional_reclassify <- terra::rast(x = file.path(mypath, "maxent_slf_gbif_v5_regional", "Lycorma_delicatula_avg.asc"))
  
  # reclassify
  v5_regional_reclassify <- terra::classify(x = v5_regional_reclassify, 
                                            rcl = regional_classes,
                                            right = NA, # close both ends of the range of classification values
                                            filename = file.path(mypath, "maxent_slf_gbif_v5_regional", "manipulated", "Lycorma_delicatula_avg_binary.asc"),
                                            overwrite = TRUE) # also write to file
  
  
  # load in global model
  v5_global_reclassify <- terra::rast(x = file.path(mypath, "maxent_slf_gbif_v5_global", "Lycorma_delicatula_avg.asc"))
  
  # reclassify
  v5_global_reclassify <- terra::classify(x = v5_global_reclassify, 
                                          rcl = global_classes, 
                                          right = NA, 
                                          filename = file.path(mypath, "maxent_slf_gbif_v5_global", "manipulated", "Lycorma_delicatula_avg_binary.asc"), 
                                          overwrite = TRUE) 
...
v5_regional_reclassify_df <- terra::rast(x = file.path(mypath, "maxent_slf_gbif_v5_regional", "manipulated", "Lycorma_delicatula_avg_binary.asc")) %>%
  terra::as.data.frame(., xy = TRUE) %>%
  na.omit()

# check for the number of unique values in the raster values column
unique(v5_regional_reclassify_df$Lycorma_delicatula_avg)
...
# plot regional
v5_regional_reclassify_plot <- ggplot() +
  geom_raster(data = v5_regional_reclassify_df, 
...
v5_regional_plot + v5_regional_reclassify_plot
...
```{r stack rasters and perform raster math}
...
if(FALSE) {
  ...
  v5_global_reclassify <- terra::rast(x = file.path(mypath, "maxent_slf_gbif_v5_global", "manipulated", "Lycorma_delicatula_avg_binary.asc"))
  
  # load in reclassified regional raster
  v5_regional_reclassify <- terra::rast(x = file.path(mypath, "maxent_slf_gbif_v5_regional", "manipulated", "Lycorma_delicatula_avg_binary.asc"))
  
  v5_global_regional <- terra::mosaic(v5_global_reclassify, v5_regional_reclassify, 
                                      fun = "sum", 
                                      filename = file.path(mypath, "maxent_slf_gbif_v5_global", "manipulated", "v5_global_regional_binary_summed.asc"))
...
```{r create map of output and overlay SLF presences}

# path to directory I will be working in
mypath <- file.path(here::here() %>% 
                          dirname(),
                        "maxent/maxent_models/maxent_slf_gbif_v5_global")

v5_global_regional_summed_df <- terra::rast(x = file.path(mypath, "manipulated", "v5_global_regional_binary_summed.asc")) %>%
  terra::as.data.frame(., xy = TRUE) %>%
  na.omit()

# check for the number of unique values in the raster values column
unique(v5_global_regional_summed_df$Lycorma_delicatula_avg)
# it seems that the reclassification worked! Lets map it to be sure.
...
# plot regional
v5_global_regional_summed_plot <- ggplot() +
  geom_raster(data = v5_global_regional_summed_df, 
              aes(x = x, y = y, fill = as.character(Lycorma_delicatula_avg))) +
