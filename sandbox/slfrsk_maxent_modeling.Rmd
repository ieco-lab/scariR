...
if(FALSE) {
  ...
  for(a in seq_along(env.files)){
    ...
    #write out the new resampled rasters!
    terra::writeRaster(x = rast.hold, filename = file.path(mypath, "v1_maxent", paste0(output.files[a], "_NAmerica")), format = "ascii", overwrite = FALSE)
...
v5_global_plot <- ggplot() +
  ...
  theme_void() +
  theme(legend.position = "none",
        title = "Predicted current global niche for \italic(Lycorma delicatula)")
