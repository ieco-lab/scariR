...
create_risk_report <- function(locality, locality.type, save.report = FALSE, mypath, create.dir = FALSE, map.style = NA) {
  ...
  # the USA is a special case, so this will be annoying
  if(locality_internal == "united_states") {

    slf_binarized_1995 <- terra::mask(
      x = slf_binarized_1995,
      mask = locality_sf_plot_layer
    )
  ...
  if(save.report == TRUE) {
    ...
    # return output
    assign("slf_risk_report", slf_risk_report, envir = .GlobalEnv)
