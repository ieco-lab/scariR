---
title: "Plot shift in SLF suitability for important viticultural regions under climate change"
...
Finally, I will load in dataset containing locations of globally important viticultural regions. I will also de-duplicate the records because there were "many-to-many" joining matches downstream. De-duplicating removed about 17 records. I also remove the species column, which is automatically created by `SDMtune::predict()` upstream.
...
```{r load in final winery pred suit files}

xy_regional_ensemble_1995 <- readr::read_rds(
  file = file.path(here::here(), "data", "regional_ensemble_wineries_1981-2010_xy_pred_suit.rds")
  )

 xy_regional_ensemble_2055_ssp_mean <- readr::read_rds(
  file = file.path(here::here(), "data", "regional_ensemble_wineries_2041-2070_GFDL_ssp_mean_xy_pred_suit.rds")
  )


# save global datasets
xy_global_1995 <- readr::read_rds(
  file = file.path(here::here(), "data", "global_wineries_1981-2010_xy_pred_suit.rds")
  )

xy_global_2055_ssp_mean <- readr::read_rds(
  file = file.path(here::here(), "data", "global_wineries_2041-2070_GFDL_ssp_mean_xy_pred_suit.rds")
  )

```

```{r create and save dataset}

# join all wineries with suitability values per model
wineries_tidied_suit <- IVR_locations %>%
  left_join(xy_global_1995, join_by(ID, x, y)) %>%
  left_join(xy_regional_ensemble_1995, join_by(ID, x, y)) %>%
  left_join(xy_global_2055_ssp_mean, join_by(ID, x, y)) %>%
  left_join(xy_regional_ensemble_2055_ssp_mean, join_by(ID, x, y)) 

# join with risk levels
wineries_tidied_suit <- wineries_tidied_suit %>%
  left_join(IVR_locations_risk, join_by(ID, x, y, Continent, Country, Region, `Sub-Region`)) %>%
  dplyr::select(-c(xy_global_1995_rescaled, xy_regional_ensemble_1995_rescaled, xy_global_2055_rescaled, xy_regional_ensemble_2055_rescaled))
...
wineries_tidied_suit <- wineries_tidied_suit %>%
  dplyr::rename(
    "global_model_suit_hist" = "xy_global_1995",
    "regional_ensemble_model_suit_hist" = "xy_regional_ensemble_1995",
    "global_model_suit_2041-2070" = "xy_global_2055",
    "regional_ensemble_model_suit_2041-2070" = "xy_regional_ensemble_2055",
...
summary_regional_ensemble_unweighted <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v1", "ensemble_threshold_values_unweighted.csv"))
...
regional_ensemble_1995_unweighted <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v1", "ensemble_regional_unweighted_mean_globe_1981-2010.asc")
  )

regional_ensemble_2055_unweighted <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v1", "ensemble_regional_unweighted_mean_globe_2041-2070_GFDL_ssp_averaged.asc")
  )
...
ggsave(
  ...
  filename = file.path(
    here::here(), "vignette-outputs", "figures", "IVR_risk_plot_regional_ensemble_unweighted.jpg"
    ),
  height = 8, 
  width = 8,
  device = "jpeg",
