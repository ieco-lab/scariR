...
(xy_joined_plot <- ggplot(data = xy_joined) +
   ...
   geom_point(
     ...
     size = 2, stroke = 0.5, color = "black", fill = "azure4"
     ) +
   # GFDL ssp370 data
   geom_point(
...
IVR_quad_percent_change <- tibble(
  ...
  "percent_change" = c(
    # extreme
    ((colSums(IVR_risk_table[, 2]) - rowSums(IVR_risk_table[1, 2:5])) / (rowSums(IVR_risk_table[1, 2:5]) + colSums(IVR_risk_table[, 2]))),
    # high
    ((colSums(IVR_risk_table[, 3]) - rowSums(IVR_risk_table[2, 2:5])) / (rowSums(IVR_risk_table[2, 2:5]) + colSums(IVR_risk_table[, 3]))),
    # moderate
    ((colSums(IVR_risk_table[, 4]) - rowSums(IVR_risk_table[3, 2:5])) / (rowSums(IVR_risk_table[3, 2:5]) + colSums(IVR_risk_table[, 4]))),
    ((colSums(IVR_risk_table[, 5]) - rowSums(IVR_risk_table[4, 2:5])) / (rowSums(IVR_risk_table[4, 2:5]) + colSums(IVR_risk_table[, 5])))
  )
)
...
(xy_joined_rescaled_plot_percent_change <- ggplot(data = xy_joined_rescaled) +
   # threshold lines
   # MTSS thresholds
   geom_vline(xintercept = global_MTSS, linetype = "dashed", linewidth = 0.6) + # global
   geom_hline(yintercept = regional_ensemble_MTSS_1995, linetype = "dashed", linewidth = 0.6) + # regional_ensemble- there are two MTSS thresholds for this model, but the difference is so small that you will never see it on the plot
   # arrows indicating change
   #geom_segment(
  #   aes(
  #     x = xy_global_1995_rescaled,
  ...
  #     yend = xy_regional_ensemble_2055_rescaled
  #   ), 
  #   arrow = grid::arrow(angle = 4.5, type = "closed"), linewidth = 0.25, color = "azure4", alpha = 0.6
     #alpha = 0.8, linewidth = 0.3, color = "azure4"
 #  ) +
   # historical data
   geom_point(
     aes(x = xy_global_1995_rescaled, y = xy_regional_ensemble_1995_rescaled, shape = "Present"), 
     size = 1.5, color = "black", fill = "azure4", stroke = 0.5, alpha = 0.1
     ) +
   # GFDL ssp370 data
   geom_point(
     aes(x = xy_global_2055_rescaled, y = xy_regional_ensemble_2055_rescaled, shape = "2041-2070\nGFDL ssp370"), 
