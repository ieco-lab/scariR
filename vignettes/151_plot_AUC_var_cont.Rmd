...
ROC_ensemble <- ggplot() +
  # native model data
  geom_line(data = regional_native_ROC$data[[1]], aes(x = x, y = y, color = "native", group = group, linetype = as.factor(group)), linewidth = 0.8) +
  # invaded model data
  geom_line(data = regional_invaded_ROC$data[[1]], aes(x = x, y = y, color = "invaded", group = group, linetype = as.factor(group)), linewidth = 0.8) +
  # invaded_asian model data
  geom_line(data = regional_invaded_asian_ROC$data[[1]], aes(x = x, y = y, color = "invaded_asian", group = group, linetype = as.factor(group)), linewidth = 0.8) +
  # midpoint line
...
jk_train_ensemble <- ggplot() +
  # native model data
  geom_col_pattern(data = regional_native_jk_train_plot$data[[1]], aes(x = x - 0.1, y = y, fill = "native", group = group, pattern = as.factor(group)), pattern_fill = "black", pattern_angle = 45, pattern_density = 0.1, pattern_spacing = 0.025, pattern_key_scale_factor = 0.6, width = 0.1, color = "black") +
  # invaded model data
