...
In the previous vignette, I ran the global model, which was an important first step to examine suitability for SLF at multiple spatial scales. In this vignette, I will set up to run three regional-scale models, one for each region where SLF is found. I will run three models, based on SLF presence data from the native range (China), the invaded region in Asia (Japan and South Korea), and the invaded region in North America. I hypothesize that by creating models at multiple spatial scales, I can have more confidence in suitability predictions made for specific SLF populations and important viticultural regions, and can predict the risk of specific populations spreading now and under climate change. I believe the regional-scale models, once ensembled into a single prediction, will make more refined predictions than a global-scale model could.

This vignette will set up for these regional-scale models in a similar fashion to the setup I created for the global model in vignette 040, but with some additional steps. I will choose the background training area of each model using the [Köppen-Geiger climate zones](https://koeppen-geiger.vu-wien.ac.at/). If a presence point intersects with a zone, it will be included in the area used to choose background points. I will also use country shapefiles to limit these K-G zones using countries that should be included in this background selection area per regional-scale model. 
...
First, I will use the [kgc](https://cran.r-project.org/web/packages/kgc/index.html) package to retrieve the Köppen-Geiger climate zones.
...
```{r plot to ensure cropping worked}
...
## Invaded region in South Korea and Japan (Ri.Asia model)
...
I just want the presences from the invaded range. I will manually filter out some points that accidentally got into both the native and Ri.Asia models.
...
## Native region in Southeast Asia (Rn model)
