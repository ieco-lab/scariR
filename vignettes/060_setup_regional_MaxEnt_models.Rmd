...
koppen_zones_rast <- terra::resample(
  x = kmz_data_rast,
  y = agg_layer, # aggregate to this origin and resoluion
  method = "mode",
  threads = TRUE,
...
(regional_invaded_withSLF_plot <- ggplot() +
   # world map
   geom_polygon(data = map_data('world', region = c("Canada", "USA", "Mexico", "Guatemala", "Honduras", "El Salvador", "Nicaragua")), aes(x = long, y = lat, group = group), fill = "azure4", color = "black", lwd = 0.15) +
   # slf native range shapefile
   geom_sf(data = regional_invaded_withSLF, aes(fill = as.factor(koeppen_climate_zones)), color = "black") +
   # scales
   labs(
     title = "SLF invaded range in NAmerica",
     x = "longitude",
     y = "latitude",
...
bio11 <- terra::rast(x = file.path(mypath, "bio11_1981-2010_regional_invaded_KG.asc")) 
# convert to df
...
(regional_invaded_asian_withSLF_plot <- ggplot() +
   ...
   # scales
   labs(
     title = "SLF invaded range within east Asia",
     x = "longitude",
     y = "latitude",
     fill = "Koeppen climate zone"
     ) +
...
# download data
southeast_asia <- rnaturalearth::ne_download(
...
slf_points_native <- read_rds(file = file.path(here::here(), "data", "slf_all_final_coords_2024-02-01.rds")) %>%
  dplyr::select(-species) %>%
  sf::st_as_sf(coords = c("x", "y"), crs = "EPSG:4326") # convert to sf object
  
STOPPED HERE

# southeast asia shapefile
southeast_asia <- southeast_asia %>%
  dplyr::filter(NAME == c("China", "Mongolia")) %>%
  # set crs
...
sf::st_write(
  ...
  dsn = file.path(here::here(), "vignette-outputs", "shapefiles", "SLF_native_range_extent.shp"),
  driver = "ESRI Shapefile"
  )
...
china_provinces_withSLF <- sf::read_sf(dsn = file.path(here::here(), "vignette-outputs", "shapefiles", "SLF_native_range_extent.shp"))
...
ggsave(native_map, 
       filename = file.path(here::here(), "vignette-outputs", "figures", "SLF_native_range_extent_v1.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina"
...
china_provinces_withSLF <- terra::vect(x = file.path(here::here(), "vignette-outputs", "shapefiles", "SLF_native_range_extent.shp"))
...
write_csv(
  ...
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_background_points_unweighted.csv")
  )
# save as rds file
write_rds(
  regional_invaded_background, 
  file = file.path(here::here(), "data", "regional_invaded_background_points_unweighted.rds")
  )
...
ggsave(
  ...
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_model_background_points_unweighted.jpg"),
  height = 8, 
  width = 12,
  device = "jpeg",
  dpi = "retina"
...
# save as csv
write_csv(x = regional_invaded_asian_points, file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_background_points_unweighted.csv"))
# save as rds file
write_rds(regional_invaded_asian_points, file = file.path(here::here(), "data", "regional_invaded_asian_background_points_unweighted.rds"))
...
regional_invaded_asian_points <- read.csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_background_points_unweighted.csv"))
...
ggsave(regional_invaded_asian_points_plot, 
       filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_model_background_points_unweighted.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina"
...
# save as csv
write_csv(x = China_points, file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_background_points_v0.csv"))
# save as rds file
write_rds(China_points, file = file.path(here::here(), "data", "regional_native_background_points_v0.rds"))
...
regional_native_points <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_background_points_v0.csv"))
...
ggsave(native_points_plot, 
       filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_model_background_points_v0.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
ggsave(
  ...
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_model_presence_points.jpg"),
  height = 8, 
  width = 10,
  device = "jpeg",
  dpi = "retina"
...
ggsave(
  ...
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_model_presence_points.jpg"),
  height = 8, 
  width = 10,
  device = "jpeg",
  dpi = "retina"
...
ggsave(
  ...
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_model_presence_points.jpg"),
  height = 8, 
  width = 10,
  device = "jpeg",
  dpi = "retina"
