...
rm(regional_ensemble_models_info.html)
...
# re-project into ESRI:54017
countries_sf <- countries_sf %>%
  sf::st_transform(src = "EPSG:4326", dst = "ESRI:54017") 
...
```{r retrieve names for zones, eval = TRUE}
...
agg_layer <- terra::rast(file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km", "bio2_1981-2010_global.asc"))
# raster stats
...
koeppen_zones_rast_plot_layer <- koeppen_zones_rast_plot_layer %>%
  dplyr::left_join(kmz_names_labels, join_by(Koeppen_climate_zone == zone_number))

# plot main layer as example
KG_climate_map <- ggplot() +
  # change scale of plots to be standard across figures
  geom_raster(data = koeppen_zones_rast_plot_layer, aes(x = x, y = y, fill = as.factor(zone_name))) +
    labs(
...
slf_points <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_final_2025-07-07.csv")) %>%
...
First, I will overlay SLF presence points and use these to select which climate regions to keep. I will use these to create a reference layer. Then, I will use this layer to mask the bioclim layers.

```{r filter presences from invaded range}

# extent object for North America
## OLD Coordinates in EPSG:4326
# ext.obj <- terra::ext(-140.976563, -51.064453, 15.182421, 60.586967)
## NEW coordinates in ESRI:54017
## coordinates converted using coordinate converter
ext.obj <- terra::ext(-13602304.166426167, -4927019.123016734, 1914870.4341456166, 6388905.495064239) # std order xmin, xmax, ymin, ymax

# convert to vector
slf_points_vect <- terra::vect(x = slf_points, geom = c("x", "y"), crs = "ESRI:54017") %>%
...
ext.obj.vect <- c(xmin = -13602304.166426167, ymin = 1914870.4341456166, xmax = -4927019.123016734, ymax = 6388905.495064239)
...
regional_invaded_withSLF <- regional_invaded_withSLF %>%
  dplyr::left_join(kmz_names_labels, join_by(koeppen_climate_zones == zone_number))
...
regional_invaded_withSLF_plot <- ggplot() +
  # world map
  geom_sf(data = countries_sf, fill = "azure4", color = "black", lwd = 0.25) +
  ...
  labs(
    ...
    caption = "K-G zones selected if containing SLF presences; grey indicates zone with no SLF",
    x = "UTM Easting",
    y = "UTM Northing",
    fill = "K-G climate\nzone"
    ) +
  ...
  coord_sf(
    # coords same as st_bbox
    xlim = c(-13383144, -5069425), 
    ylim = c(1914870, 6384927), 
    expand = FALSE, #map to fill visible bounds
    datum = "ESRI:54017",
    crs = "ESRI:54017" 
)
...
```{r save invaded K-G plot, include = FALSE}
...
First, I will crop the bioclim layers to the new extent. These layers have been cropped to the extent of a specific bounding box, which can be found in this project under root/data-raw/bounding_boxes.txt.
...
# bbox for cropping
## coords in root/data-raw/bounding_boxes.txt
ext.obj.crop <- terra::ext(-13602303.87696733, 1914870.4341456166, -4927019.123016734, 6388905.495064239, xy = TRUE) # inverse order xmin, ymin, xmax, ymax

```

I will now crop these rasters to an extent we can use.
...
## NEW coords in ESRI54017
ext.obj.crop <- terra::ext(-13379444.69115782, 2395972.663390198, -5073583.422984609, 6404430.598495737, xy = TRUE) # inverse order xmin, ymin, xmax, ymax

```

I will now mask these rasters using the shapefile created above.

```{r loop to mask bioclim layers by NAmerica invaded range}
...
```{r plot to ensure cropping worked, eval = TRUE, warning = FALSE}
...
(ggplot() +
  ...
  coord_sf(
    xlim = c(-13379444.69115782, -5073583.422984609), 
    ylim = c(2395972.663390198, 6404430.598495737), 
    expand = FALSE,
    crs = "ESRI:54017", 
    datum = "ESRI:54017"
    )
...
## NEW ESRI 54017
ext.obj <- terra::ext(11861845.759289555, 14857498.721065253, 2999956.334646307, 5227150.879943501) # std order xmin, xmax, ymin, ymax
...
slf_points_invaded_asian <- terra::as.data.frame(slf_points_vect) %>%
  ...
  dplyr::filter(x > 12060785.031362064) %>%
  # convert to sf object
  sf::st_as_sf(coords = c("x", "y"), crs = "ESRI:54017") 
...
# bounding box for cropping
ext.obj.vect <- c(xmin = 11861845.759289555, ymin = 2999956.334646307, xmax = 14857498.721065253, ymax = 5227150.879943501)
# first, crop K-G raster to SK and Japan
KG_zones_invaded_asian_poly <- sf::st_crop(KG_zones_invaded_asian_poly, y = ext.obj.vect) %>%
  # also get rid of ocean
  dplyr::filter(koeppen_climate_zones != 32)
...
```{r plot invaded_asian range, eval = TRUE, warning = FALSE}
...
regional_invaded_asian_withSLF_plot <- ggplot() +
  ...
  theme(legend.position = "right") +
  coord_sf(
    xlim = c(11861846, 14285214), 
    ylim = c(3723782, 5227151), 
    expand = FALSE, #map to fill visible bounds
    datum = "ESRI:54017",
    crs = "ESRI:54017" 
)
...
```{r save Ri.Asia K-G plot, include = FALSE}
...
# for cropping
ext.obj.crop <- terra::ext(11861845.759289555, 2999956.334646307, 14857498.721065253, 5227150.879943501, xy = TRUE) # inverse order xmin, ymin, xmax, ymax
...
```{r plot for check, eval = TRUE, warning = FALSE}
...
(ggplot() +
  ...
  theme(legend.position = "none") +
  coord_sf(
    xlim = c(11861845.759289555, 14857498.721065253), 
    ylim = c(2999956.334646307, 5227150.879943501),
    expand = FALSE, #map to fill visible bounds
    datum = "ESRI:54017",
    crs = "ESRI:54017" 
    )
...
asia_sf <- asia_sf %>%
  ...
  # set crs
asia_sf <- sf::st_transform(x = asia_sf, crs = "ESRI:54017") 
...
# convert to vector
slf_points_masked <- terra::vect(x = slf_points, geom = c("x", "y"), crs = "ESRI:54017") %>%
# crop by extent area of interest
...
# get extent
sf::st_bbox(regional_native_withSLF) 
...
```{r plot china, eval = TRUE, warning = FALSE}
...
regional_native_withSLF_plot <- ggplot() +
  ...
  theme(legend.position = "right") +
  coord_sf(
    xlim = c(7009531, 13605773), 
    ylim = c(-1221040, 5856473),
    expand = FALSE, #map to fill visible bounds
    datum = "ESRI:54017",
    crs = "ESRI:54017" 
    )
...
```{r save Rn K-G plot, include = FALSE}
...
## NEW
ext.obj.crop <- terra::ext(7009531, -1221040, 13605773, 5856473, xy = TRUE) # inverse order xmin, ymin, xmax, ymax
...
```{r plot main_layer to ensure cropping worked, eval = TRUE, warning = FALSE}
...
write_csv(
  ...
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_background_points_v8.csv")
  )
# save as rds file
write_rds(
  regional_invaded_background, 
  file = file.path(here::here(), "data", "regional_invaded_background_points_v8.rds")
)
...
regional_invaded_background <- read.csv(file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_background_points_v8.csv")) 

```

```{r plot Ri.NAmerica background, eval = TRUE, echo = FALSE}

regional_invaded_background_plot <- ggplot() +
  map_style +
  # empty world map to force same aspect ratio
  geom_sf(data = countries_sf, fill = NA, color = NA) +
  # base layer
...
```{r save Ri.Namerica background plot, include = FALSE}

ggsave(
  regional_invaded_background_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_model_background_points_v8.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
# save as csv
write_csv(x = regional_invaded_asian_points, file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_background_points_v3.csv"))
# save as rds file
write_rds(regional_invaded_asian_points, file = file.path(here::here(), "data", "regional_invaded_asian_background_points_v3.rds"))
...
```{r load in data for plotting and change projection- Ri.Asia, eval = TRUE, message = FALSE, warning = FALSE}
...
# bg points
regional_invaded_asian_points <- read.csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_background_points_v3.csv")) 

```

```{r plot Ri.Asia bg, eval = TRUE, echo = FALSE}
...
regional_invaded_asian_points_plot <- ggplot() +
  ...
  coord_sf(crs = "ESRI:54017", datum = "ESRI:54017") +
  scale_x_continuous(limits = c(11861846, 14285214), expand = c(0, 0)) +
  scale_y_continuous(limits = c(3723782, 5227151), expand = c(0, 0))

regional_invaded_asian_points_plot

```

```{r save plot- Ri.Asia bg, include = FALSE}

ggsave(
  regional_invaded_asian_points_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_model_background_points_v3.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
```{r background points for Rn model}
...
write_csv(x = regional_native_points, 
          file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_background_points_v4.csv")
          )
# save as rds file
write_rds(regional_native_points, 
          file = file.path(here::here(), "data", "regional_native_background_points_v4.rds")
)
  
  
```

```{r load in files for plotting and change projection- Rn, eval = TRUE, message = FALSE, warning = FALSE}

regional_native_bio2_df <- terra::rast(x = file.path(mypath, "bio2_1981-2010_regional_native_KG.asc")) %>%
  terra::as.data.frame(., xy = TRUE)


regional_native_points <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_background_points_v4.csv")) 
...
```{r save plot- Rn bg, include = FALSE}

ggsave(
  regional_native_points_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_model_background_points_v4.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
ext.obj <- terra::ext(-13383144, -5069425, 1914870, 6384927)
...
write_csv(
  ...
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_train_slf_presences_v8.csv")
)
...
```{r plot model presences- Ri.NAmerica, eval = TRUE, echo = FALSE}

# load in raster
regional_invaded_df <- terra::rast(x = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km", "bio2_1981-2010_regional_invaded_KG.asc")) %>%
  terra::as.data.frame(., xy = TRUE)

# re-import point data
slf_points_invaded <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_train_slf_presences_v8.csv")) 
...
```{r save Ri.NAmerica presence plot, include = FALSE}

ggsave(
  regional_invaded_slf_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_train_slf_presences_v8.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
```{r save presences- Ri.Asia, include = FALSE}
...
```{r plot model presences- Ri.Asia, eval = TRUE, echo = FALSE}

invaded_asian_df <- terra::rast(
  x = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km", "bio2_1981-2010_regional_invaded_asian_KG.asc")
  ) %>%
  terra::as.data.frame(., xy = TRUE)

# re-import point data
slf_points_invaded_asian <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_train_slf_presences_v3.csv")) 
...
```{r save Ri.Asia presence plot, include = FALSE}

ggsave(
  regional_invaded_asian_slf_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_train_slf_presences_v3.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
# native range extent shapefile used to select slf presences
mask_layer <- terra::vect(x = file.path(here::here(), "vignette-outputs", "shapefiles", "SLF_regional_native_extent.shp"))
...
write_csv(
  x = slf_points_native,
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_train_slf_presences_v4.csv")
)

```

```{r plot model presences- Rn, eval = TRUE, echo = FALSE}

# raster
regional_native_df <- terra::rast(
  x = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km", "bio2_1981-2010_regional_native_KG.asc")
  ) %>%
  terra::as.data.frame(., xy = TRUE)

# re-import point data
slf_points_native <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_train_slf_presences_v4.csv"))
...
```{r save Rn presence plot, include = FALSE}

ggsave(
  regional_native_slf_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_train_slf_presences_v4.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
# select 20% of training total (150 points) 

set.seed(8)

# randomly select subset of rows for testing
slf_points_invaded_test_rows <-  sample(nrow(slf_points_invaded_asian), size = 150, replace = FALSE) # no replacement
# filter
...
write_csv(
  ...
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_test_slf_presences_v8.csv")
)
...
# select 20% of training total (43 points) 

set.seed(9)

# randomly select subset of rows for testing
slf_points_invaded_asian_test_rows <-  sample(nrow(slf_points_invaded), size = 43, replace = FALSE) # no replacement
# filter
...
# select 20% of training total (64 points) 

set.seed(10)

# randomly select subset of rows for testing
slf_points_native_test_rows <-  sample(nrow(slf_points_invaded), size = 64, replace = FALSE) # no replacement
# filter
...
write_csv(
  ...
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_test_slf_presences_v4.csv")
)
...
10. Coordinate Converter – Polar Geospatial Center [Internet]. Available from: https://applications.pgc.umn.edu/convert/
...
invaded_asian_df <- terra::rast(x = file.path(mypath, "bio2_1981-2010_regional_invaded_asian_KG.asc")) %>%
  terra::as.data.frame(., xy = TRUE)
...
# labels
x_scale_labels <- seq(from = -13000000, to = -6000000, by = 2000000)
y_scale_labels <- seq(from = 2000000, to = 6000000, by = 1000000)
...
# new labels
x_scale_labels <- seq(from = 12000000, to = 14000000, by = 5000000)
y_scale_labels <- seq(from = 3800000, to = 5200000, by = 200000)
...
regional_invaded_test_plot <- regional_invaded_test_plot +
  ...
  labs(tag = "D") +
  scale_x_continuous(limits = c(11861846, 14285214), labels = x_scale_labels, breaks = x_scale_labels, expand = c(0, 0)) +
  scale_y_continuous(limits = c(3723782, 5227151), labels = y_scale_labels, breaks = y_scale_labels, expand = c(0, 0)) +
  theme(
    plot.tag.location = "panel",
    plot.tag.position = c(0.05, 0.1),
    plot.tag = element_text(face = "bold", size = 25)
...
ggsave(
  regional_invaded_patchwork, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_model_v8_summary.jpg"),
  height = 10, 
  width = 14,
  device = jpeg,
  dpi = 500
...
regional_invaded <- terra::rast(
  x = file.path(mypath, "bio2_1981-2010_regional_invaded_KG.asc")
  ) %>%
  terra::as.data.frame(., xy = TRUE)
...
# labels
x_scale_labels <- seq(from = 12000000, to = 14000000, by = 500000)
y_scale_labels <- seq(from = 3800000, to = 5200000, by = 200000)
...
ggsave(
  regional_invaded_asian_patchwork, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_model_v3_summary.jpg"),
  height = 10, 
  width = 12,
  device = jpeg,
  dpi = 500
...
x_scale_labels <- seq(from = 8000000, to = 13000000, by = 1000000)
y_scale_labels <- seq(from = -1000000, to = 5000000, by = 1000000)
...
ggsave(
  regional_native_patchwork, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_model_v4_summary.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
