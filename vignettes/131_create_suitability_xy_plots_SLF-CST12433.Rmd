...
if (FALSE) {
  ...
  readr::write_rds(slf_populations, file.path(here::here(), "data", "slf_all_final_coords_2024-02-01_tidied.rds"))
  
  #read it back in
  slf_populations <- read_rds(file.path(here::here(), "data", "slf_all_final_coords_2024-02-01_tidied.rds"))
...
write_csv(
  ...
  file = file.path(mypath, "slf_regional_ensemble_v1", "regional_ensemble_slf_all_coords_1981-2010_xy_pred_suit.csv")
  )
# 2055
write_csv(
  x =  xy_regional_ensemble_2055,
  file.path(mypath, "slf_regional_ensemble_v1", "regional_ensemble_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit.csv")
  )
...
xy_global_2055 <- read_rds(file = file.path(here::here(), "data", "global_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit.rds"))
...
xy_regional_ensemble_2055 <- read_rds(file = file.path(here::here(), "data", "regional_ensemble_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit.rds"))
...
write_rds(
  ...
  file = file.path(here::here(), "data", "global_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit_rescaled.rds")
  )
...
write_rds(
  ...
  file = file.path(here::here(), "data", "regional_ensemble_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit_rescaled.rds")
  )
...
write_rds(
  ...
  file = file.path(here::here(), "data", "global_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit_rescaled_thresholds.rds")
  )
...
write_rds(
  ...
  file = file.path(here::here(), "data", "regional_ensemble_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit_rescaled_thresholds.rds")
  )
...
ggsave(
  xy_joined_plot, 
  filename = file.path(
    here::here(), "vignette-outputs", "figures", "slf_xy_suitability_slf_populations_global_regional_ensemble.jpg"
    ),
  height = 8, 
  width = 8,
  device = "jpeg",
...
write_rds(
  xy_joined_rescaled_plot,
  file = file.path(here::here(), "vignette-outputs", "figures", "slf_risk_plot.rds")
)
...
slf_risk_table <- slf_risk_table %>%
  ...
  # add row totals
  tibble::add_row(rows_1995_cols_2055 = "total_2055", extreme = colSums(dplyr::select(., 2)), high = colSums(dplyr::select(., 3)), moderate = colSums(dplyr::select(., 4)), low = colSums(dplyr::select(., 5)), total_present = 769) 
