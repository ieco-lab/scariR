...
summary_regional_ensemble <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v0", "ensemble_threshold_values_unweighted.csv"))
...
xy_global_1995_rescaled <- slfSpread::TEST_rescale_cloglog_suitability(
...
xy_global_2055_rescaled <- slfSpread::TEST_rescale_cloglog_suitability(
...
xy_regional_ensemble_1995_rescaled <- slfSpread::TEST_rescale_cloglog_suitability(
...
xy_regional_ensemble_2055_rescaled <- slfSpread::TEST_rescale_cloglog_suitability(
...
(xy_joined_plot <- ggplot(data = xy_joined) +
   ...
   geom_segment(
     ...
     arrow = grid::arrow(angle = 8, type = "closed"), linewidth = 0.25, color = "azure4", alpha = 0.6
   ) +
   # historical data
   geom_point(
     aes(x = xy_global_1995, y = xy_regional_ensemble_1995, color = "1981-2010"), 
     size = 1
     ) +
   # GFDL ssp370 data
   geom_point(
     aes(x = xy_global_2055, y = xy_regional_ensemble_2055, color = "2041-2070\nGFDL ssp370"), 
     size = 1
     ) +
   # axes scaling
...
(xy_joined_rescaled_plot <- ggplot(data = xy_joined_rescaled) +
   ...
   # historical data
   geom_point(
     aes(x = xy_global_1995_rescaled, y = xy_regional_ensemble_1995_rescaled, color = "1981-2010"), 
     size = 1
     ) +
   # GFDL ssp370 data
   geom_point(
     aes(x = xy_global_2055_rescaled, y = xy_regional_ensemble_2055_rescaled, color = "2041-2070\nGFDL ssp370"), 
     size = 1
     ) +
   # axes scaling
   scale_x_continuous(name = "'global' model suitability", limits = c(0, 1), breaks = breaks) + 
...
regional_ensemble_MTSS <- as.numeric(xy_regional_ensemble_1995_rescaled_thresholds[2, 2])

 quad_counts <- tibble(
   # the axis value of this 
   axis = c("x_global", "y_regional_ensemble"),
   # mean values of each of the x and y axes according to the threshold
   # top right quadrant
   high_risk_quad4_1995 = c(
     xy_joined_rescaled %>% dplyr::select(xy_global_1995_rescaled) %>% dplyr::filter(xy_global_1995_rescaled >= global_MTSS) %>% colMeans(),
     xy_joined_rescaled %>% dplyr::select(xy_regional_ensemble_1995_rescaled) %>% dplyr::filter(xy_regional_ensemble_1995_rescaled >= regional_ensemble_MTSS) %>% colMeans()
   ),
   # top left quadrant
   adaptive_risk_quad3_1995 = c(
     xy_joined_rescaled %>% dplyr::select(xy_global_1995_rescaled) %>% dplyr::filter(xy_global_1995_rescaled < global_MTSS) %>% colMeans(),
     xy_joined_rescaled %>% dplyr::select(xy_regional_ensemble_1995_rescaled) %>% dplyr::filter(xy_regional_ensemble_1995_rescaled >= regional_ensemble_MTSS) %>% colMeans()
   ),
   # bottom right quadrant
   imminent_risk_quad2_1995 = c(
     xy_joined_rescaled %>% dplyr::select(xy_global_1995_rescaled) %>% dplyr::filter(xy_global_1995_rescaled >= global_MTSS) %>% colMeans(),
     xy_joined_rescaled %>% dplyr::select(xy_regional_ensemble_1995_rescaled) %>% dplyr::filter(xy_regional_ensemble_1995_rescaled < regional_ensemble_MTSS) %>% colMeans()
   ),
   # bottom right quadrant
