...
The goal is to produce four models: a global model, a native range model, and two invaded models based on the invaded ranges in North America and east Asia. All three models will be based on historical (1981-2010) climate data and will be extrapolated for climate change using the CMIP6 rasters. Each model will be based on different training and testing ranges. The global model will be trained and tested on all available climate data, which will be split up into 10 random folds. The regional models will be trained on the regional subset of presence data. The regional_native model will be trained on SLF presences and background points chosen from provinces in China which contain known SLF presences. This model will be tested using SLF presence data from the USA. The invaded models will do the opposite: they will be trained on the data from the USA and tested using data from the native range in China. All three of these models will be considered in tandem during analyses. Here is a table that explains the models. The following table outlines the training and testing of these models:
...
## 1.1 Historical Rasters- N America
...
if (FALSE) {
  ...
  # save as .csv
  write_csv(model_info, file.path(here::here(), "vignette-outputs", "data-tables", "global_model_info.csv"))
  
  # save as .html
  kableExtra::save_kable(
    model_info_kable, 
    file = file.path(here::here(), "vignette-outputs", "figures", "global_model_info.html"),
    self_contained = TRUE,
    bs_theme = "simplex"
    )
 ...
 # webshot::install_phantomjs()
  # convert to pdf
  webshot::webshot(
    url = file.path(here::here(), "vignette-outputs", "figures", "global_model_info.html"),
    file = file.path(here::here(), "vignette-outputs", "figures", "global_model_info.png")
  )
...
I will repeat this process for the other SSP scenarios as well
...
# path to directory
  mypath <- file.path(here::here() %>% 
                       dirname(),
                     "maxent/future_climate_rasters/chelsa2.1_30arcsec/2041-2070_ssp126_GFDL/v1_maxent_10km")
...
# extract the 4 bioclim layers I will be using in my models. 
  grep("bio2_2041-2070_gfdl_126_global.asc|bio11_2041-2070_gfdl_126_global.asc|bio12_2041-2070_gfdl_126_global.asc|bio15_2041-2070_gfdl_126_global.asc", ., value = TRUE)
...
# path to directory
  mypath <- file.path(here::here() %>% 
                       dirname(),
                     "maxent/future_climate_rasters/chelsa2.1_30arcsec/2041-2070_ssp585_GFDL/v1_maxent_10km")
...
env.files <- list.files(path = file.path(mypath), pattern = "\\.asc$", full.names = TRUE) %>%
# extract the 4 bioclim layers I will be using in my models. 
  grep("bio2_2041-2070_gfdl_585_global.asc|bio11_2041-2070_gfdl_585_global.asc|bio12_2041-2070_gfdl_585_global.asc|bio15_2041-2070_gfdl_585_global.asc", ., value = TRUE)
...
slf_points <- read_rds(file = file.path(here::here(), "data", "slf_all_final_coords_2024-07-29.rds")) %>%
...
# save as csv
write_csv(x = global_points, file = file.path(here::here(), "vignette-outputs", "data-tables", "global_background_points_v3.csv"))
# save as rds file
write_rds(global_points, file = file.path(here::here(), "data", "global_background_points_v3.rds"))
...
global_points <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "global_background_points_v3.csv"))
...
ggsave(
  ...
  filename = file.path(here::here(), "vignette-outputs", "figures", "global_model_background_points.jpg"),
  height = 8, 
  width = 12,
  device = "jpeg",
  dpi = "retina"
...
global_slf_plot <- ggplot() +
  geom_raster(data = global_bio2_df, aes(x = x, y = y), fill = "azure1") +
  geom_point(data = slf_points, aes(x = x, y = y), color = "darkorange", size = 0.1) +
  labs(
    title = "'global' model SLF presences",
    x = "longitude",
    y = "latitude"
  ) +
  scale_x_continuous(expand = c(0, 0)) +
...
ggsave(
  global_slf_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "global_model_presence_points.jpg"),
  height = 8, 
  width = 12,
  device = "jpeg",
  dpi = "retina"
...
# Patchwork background and presence plots
...
global_patchwork <- (global_slf_plot / global_background_plot) +
  ...
  plot_annotation(title = "'global' model presences (A) and background points (B)") 
...
ggsave(
  global_patchwork, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "global_model_summary.jpg"),
  height = 8, 
  width = 10,
  device = "jpeg",
  dpi = "retina"
