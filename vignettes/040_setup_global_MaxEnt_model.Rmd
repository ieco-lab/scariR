...
*Note* For all mapping applications moving forward, I will be re-projecting all data into the [Behrmann cylindrical equal-area projection](https://epsg.io/54017), `ESRI:54017`. Part of my analysis emphasizes distributional areas, so use of an equal-area projection is important for preserving and calculating area correctly. I will transform all maps into this projection for consistency.
...
global_bio2 <- terra::rast(x = file.path(mypath, "bio2_1981-2010_global.asc")) 

global_points_raw <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "global_background_points_v3.csv"))

```

This is my first visualization of the component data for the models, so I will be re-projecting these data to 

```{r reproject raster and point data}

# reproject raster 
global_bio2 <- terra::project(x = global_bio2, y = "ESRI:54017", method = "bilinear", threads = TRUE)
# check crs
crs(global_bio2, parse = TRUE)
# convert to df
global_bio2_df <- terra::as.data.frame(global_bio2, xy = TRUE) 


# transform coordinate data to UTM
# convert to vector
global_points <- global_points_raw %>%
  terra::vect(., geom = c("x", "y"), crs = "EPSG:4326") %>%
  # project coordinates
  terra::project(., y = "ESRI:54017") %>%
  # use geom to get coordinates of a spatVector and convert to df
  terra::geom(., df = TRUE) %>%
...
```{r plot model presences, eval = TRUE}
...
# Appendix- Patchwork background and presence plots {.hidden}
