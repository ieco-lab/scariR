...
bio11 <- terra::rast(x = file.path(mypath, "v1_maxent_10km", "bio11_1981-2010_easternUSA.asc")) 
# convert to df
bio11_df <- terra::as.data.frame(bio11, xy = TRUE)
...
bio11_AUC <- terra::rast(x = file.path(mypath, "v1_maxent_10km", "bio11_1981-2010_NAmerica.asc")) 
...
write_csv(x = fixed_AUC, file = file.path(here(), "vignette-outputs", "data-tables", "NAmerica_fixed_area_points_v1.csv"))
...
fixed_AUC <- read_csv(file = file.path(here(), "vignette-outputs", "data-tables", "NAmerica_fixed_area_points_v1.csv"))
  ...
  fixed_AUC_plot <- ggplot() +
   ...
   #geom_polygon(data = map_data("county"), aes(x = long, y = lat, group = group), fill = NA, color = "black", lwd = 0.15) + # county outlines
    geom_point(data = fixed_AUC, 
               aes(x = x, y = y), color = "darkorange", size = 0.25) +
...
entire_USA <- terra::rast(x = file.path(mypath, "chelsa2.1_30arcsec", "v1_maxent_10km", "bio11_1981-2010_easternUSA.asc")) 
...
### 1% adult emergence area of Eastern USA
...
### 50% Egg laying area of Eastern USA
