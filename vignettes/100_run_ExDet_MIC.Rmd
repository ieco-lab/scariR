...
```{r load in historical covariates, eval = TRUE}
...
```{r load in CMIP6 covariates, eval = TRUE}
...
```{r stack historical covariates and make naming consistent, message = FALSE, eval = TRUE}
...
```{r stack CMIP6 covariates, message = FALSE, eval = TRUE}
...
```{r extract CMIP6 values for background points- SSP126, eval = TRUE}
...
```{r extract CMIP6 values for background points- SSP370, eval = TRUE}
...
```{r extract CMIP6 values for background points- SSP585, eval = TRUE}
...
```{r select only data columns, eval = TRUE}
...
```{r convert projection rasters to df, eval = TRUE}
...
```{r regional_native model, eval = TRUE}
...
```{r regional_invaded model, eval = TRUE}
...
```{r regional_invaded_asian model, eval = TRUE}
...
```{r regional_native model- SSP126, eval = TRUE}
...
```{r regional_invaded model- SSP126, eval = TRUE}
...
```{r regional_invaded_asian model- SSP126, eval = TRUE}
...
```{r global model- SSP126, eval = TRUE}
...
```{r regional_native model- SSP370, eval = TRUE}
...
```{r regional_invaded model- SSP370, eval = TRUE}
...
```{r regional_invaded_asian model- SSP370, eval = TRUE}
...
```{r global model- SSP370, eval = TRUE}
...
```{r regional_native model- SSP585, eval = TRUE}
...
```{r regional_invaded model- SSP585, eval = TRUE}
...
```{r regional_invaded_asian model- SSP585, eval = TRUE}
...
```{r global model- SSP585, eval = TRUE}
...
exdet_regional_native_hist_plot <- map_exdet(
  ...
  raster.df = exdet_MIC_regional_native_hist[["data"]][["all"]]
  ) 
