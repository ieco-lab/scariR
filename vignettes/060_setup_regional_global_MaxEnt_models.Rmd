...
I will select 50000 random background points using `dismo::randomPoints()`. This amount was recommended when modeling at a global scale (Santana Jr et.al, 2019). Background points datasets will be stored in `vignette-outputs/data-tables`.
...
global_background_points <- dismo::randomPoints(mask = bio11_global, 
                                                n = 50000, # number recommended for global studies
                                                p = slf_points,
...
# save as csv
write_csv(x = global_background_points, file = file.path(here(), "vignette-outputs", "data-tables", "global_background_points.csv"))
# save as rda file
save(global_background_points, file = file.path(here(), "data", "global_background_points.rda"))
  
```

The initial run threw a warning that only 0.85046 times 50,000 points could be selected. This came to 42,523 points in total. I will not try to plot this, because it crashed my PC when I tried.
