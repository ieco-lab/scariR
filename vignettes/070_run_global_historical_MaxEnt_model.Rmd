---
title: "Run global historical maxent model (project step 2)"
...
This vignette will build off of `050_run_easternUSA_MaxEnt_models.Rmd`, which comprised step 1 of my analysis. In step 1, I tested the best fit background data using SLF occurrences and climate data only from the SLF invaded range (the eastern half of the United States). There was no significant difference between the models, so we selected the model that made the fewest assumptions about factors that limit SLF establishment, which used the entire eastern USA to select pseudoabsences (easternUSA_entire_model). I will create a global version of this model, which will be trained on all known SLF presences and the global area climate rasters. 
...
x_NAmerica_env_covariates_list <- list.files(path = file.path(mypath, "v1_maxent"), pattern = "\\_NAmerica.asc$", full.names = TRUE) %>%
...
a_global_background_points <- read.csv(file.path(here(), "vignette-outputs", "data-tables", "global_background_points_v1.csv"))
...
# NAmerica rasters
# stack env covariates
x_NAmerica_env_covariates <- terra::rast(x = x_NAmerica_env_covariates_list)

# attributes
nlyr(x_NAmerica_env_covariates)
names(x_NAmerica_env_covariates)
minmax(x_NAmerica_env_covariates)
# ext(x_NAmerica_env_covariates)
# crs(x_NAmerica_env_covariates)

names(x_NAmerica_env_covariates) <- env_layer_names
...
if (TRUE) {
  ...
  global_historical_SWD <- SDMtune::prepareSWD(species = "Lycorma delicatula",
  ...
  global_historical_SWD@coords # coordinates
  global_historical_SWD@pa # presence / absence (background counted as absence)
  global_historical_SWD@data # extracted data from 
...
SDMtune::swd2csv(swd = global_historical_SWD, file_name = c(
  file.path(here(), "vignette-outputs", "data-tables", "global_historical_slf_presences_with_data_v1.csv"),
  file.path(here(), "vignette-outputs", "data-tables", "global_historical_absences_with_data_v1.csv")
  ))
...
Radosavljevic and Anderson used a range of regularization multipliers, from 0.25 to 2
