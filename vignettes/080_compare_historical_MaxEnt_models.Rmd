...
# global raster
global_historical_mean <- terra::rast(x = file.path(mypath, "slf_global_historical_step2", ""))
# convert to df for plotting
global_historical_mean_df <- terra::as.data.frame(x = global_historical_mean, xy = TRUE)

# regional raster
regional_historical_mean <- terra::rast(x = file.path(mypath, "slf_easternUSA_entire_step1_v1", ""))
# convert to df for plotting
regional_historical_mean_df <- terra::as.data.frame(x = regional_historical_mean, xy = TRUE)

```


# Analyze suitability values for global vs regional models
...
# suitability values
# regional
slf_suit_regional <- read_csv(file = file.path(mypath, "slf_easternUSA_entire_step1_v1", "easternUSA_entire_slf_all_coords_v1_predicted_suitability_mean.csv")) %>%
  dplyr::rename("regional_suitability" = "cloglog_suitability")
# global
slf_suit_global <- read_csv(file = file.path(mypath, "slf_global_historical_step2", "global_historical_slf_all_coords_v1_predicted_suitability_mean.csv")) %>%
...
# regional
summary_regional <- read_csv(file = file.path(mypath, "slf_easternUSA_entire_step1_v1", "easternUSA_entire_summary_all_iterations.csv"))
# global
summary_global <- read_csv(file = file.path(mypath, "slf_global_historical_step2", "global_historical_summary_all_iterations.csv"))
...
ggsave(slf_suit_scatter, filename = file.path(here(), "vignette-outputs", "figures", "slf_suitability_historical_models.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
# suitability values
# regional
IVR_suit_regional <- read_csv(file = file.path(mypath, "slf_easternUSA_entire_step1_v1", "easternUSA_entire_wineries_predicted_suitability_mean.csv")) %>%
  dplyr::rename("regional_suitability" = "cloglog_suitability")
# global
IVR_suit_global <- read_csv(file = file.path(mypath, "slf_global_historical_step2", "global_historical_wineries_predicted_suitability_mean.csv")) %>%
  dplyr::rename("global_suitability" = "cloglog_suitability")
