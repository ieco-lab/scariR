...
The EPGS number that matches the previous `proj4` string is `EPSG:4326`. We will use this notation from now on. 
...
I have found that choosing covariates can be rather ad-hoc, so I developed what I believe is the best-fit methodology for choosing covariates out of much practice. First, I look at the different groupings of covariates (those based on temperature, precipitation and degree-day) and choose a representative from each group. The ideal representative is highly correlated with most of the others in the group, which indicates its predictive power (SOURCE), and represents a generality of its group (ie, the mean yearly average). In this case, I chose annual mean temp and precip (bio 1 and 12). None of the GDD variables were chosen because they were all highly correlated with annual mean temp or each other. Summed growing degree days was the exception to this rule, because it is not highly correlated with annual mean temp. However, it was not chosen because GDD is being utilized downstream. From this, I can also choose atc because it is not correlated with any other variables. 
...
Indeed, the choice is much clearer on this plot. Annual mean temp (bio 1) is above 0.7 correlated with all other temp-based variables, except mean diurnal range (bio 2) and temperature annual range (bio 7). Meanwhile, annual precipitation is highly correlated with all precipitation based variables, except precipitation seasonality (bio 12). These three variables were also chosen. 
