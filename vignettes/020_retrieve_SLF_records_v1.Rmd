...
The raw query and the raw coordinates were saved to the `data-raw` folder. I will read in the coordinate data that was just saved in preparation for data tidying. 

I will exclude SLF records from North America and only keep records from Asia. North American records are already represented by "lydemapr", but the Asia records are not and need to be kept. I will exclude any records in Canada, Mexico or the USA. Official ISO 3166 country abbreviations were found at: https://www.iban.com/country-codes.

```{r read in raw data}

# re-read in slf datasets
# raw query
slf_gbif_final <- read_csv(file = file.path(here(), "vignette-outputs", "data-tables", "slf_gbif_2023-11-12.csv"))
# occ2df version
...
# find unique countries listed
unique(slf_gbif_final_key$countryCode)

# select only Asia records
slf_gbif_coords1 <- slf_gbif_coords1 %>%
  ...
  filter(!is.na(countryCode), # dont select records where country of record is unknown
         countryCode != "US", 
         countryCode != "MX", 
         countryCode != "CA" # don't select N America records
         ) %>%
...
if(TRUE) {
  ...
  slf_all_coords2 <- spThin::thin(loc.data = slf_all_coords, 
                              ...
                              write.log.file = TRUE, 
                              log.file = file.path(here(), "vignette-outputs", "data-tables", paste0("slf_all_coords_cleaned_thinning_log_2023-11-12.txt")),
                              ...
                              out.base = paste0("slf_all_coords_cleaned_2023-11-12"),
...
# save
write_csv(x = slf_all_coords3, file = file.path(here(), "vignette-outputs", "data-tables", paste0("slf_all_final_coords_2023-11-12.csv")))

# also save as a .rda file
save(slf_all_coords3, file = file.path(here(), "data", paste0("slf_all_final_coords_2023-11-12.rda")))
