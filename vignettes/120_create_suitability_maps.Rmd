---
title: "Create risk maps of each model scale and intersect predictions"
...
```{r map style binarized, eval = TRUE}
...
```{r ensemble rasters, eval = TRUE, message = FALSE}
...
```{r ensemble thresholds, eval = TRUE, message = FALSE}

regional_ensemble_thresholds <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v1", "ensemble_threshold_values.csv"))

```

```{r change raster names and convert to df- regional, eval = TRUE, message = FALSE}
...
```{r global model rasters, eval = TRUE, message = FALSE}
...
```{r summary file, eval = TRUE, message = FALSE}
...
```{r change raster names and convert to df- global, eval = TRUE, message = FALSE}
...
```{r mapping helper function, eval = TRUE}
...
```{r map regional ensemble: 1981-2010, eval = TRUE}
...
```{r map global: 1981-2010, eval = TRUE, echo = FALSE}
...
```{r save plot- global hist, include = FALSE}
...
```{r map regional ensemble: ssp mean, eval = TRUE, echo = FALSE}
...
```{r save plot- ensemble ssp mean, include = FALSE}
...
```{r map global: ssp_mean, eval = TRUE, echo = FALSE}
...
```{r save plot- global ssp_mean, include = FALSE}
...
```{r recall threshold values, eval = TRUE}
...
```{r minmax of rasters- hist, eval = TRUE}
...
```{r terra required classification matrices- hist, eval = TRUE}
...
```{r prepare for plotting- hist, eval = TRUE}
...
```{r plot binarized raster- hist, eval = TRUE}
...
```{r helper function- terra classification matrices, eval = TRUE}
...
```{r minmax of rasters- ssp mean, eval = TRUE}
...
```{r terra required classification matrices- ssp mean, eval = TRUE}
...
```{r prepare for plotting- ssp mean, eval = TRUE}
...
```{r plot binarized raster- ssp mean, eval = TRUE}
...
```{r prepare data for mapping- hist, eval = TRUE}
...
```{r plot summed raster- hist, eval = TRUE}
...
```{r helper function and plotting objects, eval = TRUE}
...
```{r prepare data for mapping- ssp mean, eval = TRUE}
...
```{r plot summed raster- ssp mean, eval = TRUE}
...
```{r retrieve summary table- area sums, eval = TRUE}
...
```{r format and convert to Kable, eval = TRUE}
...
```{r helper function- plot range shifts, eval = TRUE}
...
```{r helper function- create range shift table, eval = TRUE}
...
```{r prepare data for mapping- range shift ssp126, eval = TRUE}
...
```{r plot summed raster- range shift ssp126, eval = TRUE}
...
```{r retrieve summary table- ssp126 range shift, eval = TRUE}
...
```{r format range shift table and kable, eval = TRUE}
...
```{r prepare data for mapping- range shift ssp mean, eval = TRUE}
...
```{r plot summed raster- range shift ssp mean, eval = TRUE}
...
```{r retrieve summary table- ssp mean range shift, eval = TRUE}
