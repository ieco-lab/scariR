...
if(FALSE) {
  ...
  regional_ensemble_1995_classified <- terra::classify(
    ...
    filename = file.path(mypath, "working_dir", "slf_regional_ensemble_v0_binarized.asc"), # also write to file
    filetype = "AAIGrid",
    overwrite = TRUE
    ) 
  ...
  global_1995_classified <- terra::classify(
    ...
    filename = file.path(mypath, "working_dir", "slf_global_v2_binarized.asc"), 
    filetype = "AAIGrid",
    overwrite = TRUE
    ) 
...
regional_ensemble_1995_classified_df <- terra::rast(
  x = file.path(mypath, "working_dir", "slf_regional_ensemble_v0_binarized.asc")
  ) %>%
...
ggplot() +
  geom_raster(data = regional_ensemble_1995_classified_df, aes(x = x, y = y, fill = as.character(ensemble_suitability))) +
  labs(title = "Binarized suitability for 'slf_regional_ensemble_v0' model") +
