...
In this vignette, I will compare known SLF presences with the predicted SLF suitability to infer a level of establishment risk for each point. This will be done by extracting the predicted suitability values at each known SLF point from both models. I will then binarize these rasters (suitable/unsuitable) and use these versions to infer the level of range filling and establishment risk of the points. 

For my analysis, I need to compare the predictions of the global model and all regional models. I will begin by comparing the performance of each model at each point across the invaded range. Gallien et.al did this for both observed presences and pseudo-absences ("background points"), but in this vignette I will only use known SLF presences. The extracted values of the points were used to produce a boxplot of predicted suitability for the global vs the regional model. I will create a boxplot of the values and plot the points over the rasters
...
regional_ensemble_2055_mean <- terra::rast(
...
names(regional_ensemble_2055_mean) <- "ensemble_suitability"
...
regional_ensemble_2055_mean_df <- terra::as.data.frame(regional_ensemble_2055_mean, xy = TRUE)
...
names(global_2055_mean) <- "global_suitability"
...
global_2055_mean_df <- terra::as.data.frame(global_2055_mean, xy = TRUE)
...
regional_ensemble_1995_plot <- map_cloglog_output(
  ...
  thresh.MTSS = regional_ensemble_thresholds[5, 4] # the df cell coordinates of the MTSS thresh
) +
  # labs
  labs(
...
regional_ensemble_2055_370_plot <- regional_ensemble_2055_370_plot <- map_cloglog_output(
...
global_2055_classified <- terra::rast(x = file.path(mypath, "working_dir", "slf_global_v3_binarized_2041-2070_ssp370_GFDL.asc"))

regional_ensemble_2055_classified <- terra::rast(x = file.path(mypath, "working_dir", "slf_regional_ensemble_v1_binarized_2041-2070_ssp370_GFDL.asc"))
...
slf_range_shift_plot <- ggplot() +
  ...
  labs(title = "Projected areas suitable for Lycorma delicatula range expansion by 2055") +
  scale_discrete_manual(
...
if(FALSE) {
  ...
  # save as .csv
  write_csv(slf_range_shift_table, file.path(here::here(), "vignette-outputs", "data-tables", "slf_range_shift_summed_area_table.csv"))
  
  # save as .html
  kableExtra::save_kable(
    slf_range_shift_kable, 
    file = file.path(here::here(), "vignette-outputs", "figures", "slf_range_shift_summed_area_table.html"),
    self_contained = TRUE,
    bs_theme = "simplex"
    )
 ...
 # webshot::install_phantomjs()
  # convert to pdf
  webshot2::webshot(
    url = file.path(here::here(), "vignette-outputs", "figures", "slf_range_shift_summed_area_table.html"),
    file = file.path(here::here(), "vignette-outputs", "figures", "slf_range_shift_summed_area_table.jpg")
  )
...
# plot
slf_binarized_2055_plot_withCountries <- slf_binarized_2055_plot +
  geom_sf(data = global_countries, aes(geometry = geometry), fill = NA, color = "black", linewidth = 0.15) + 
  # re-crop to usual boundaries
  coord_sf(xlim = c(-180.00013888885, 178.999859675084), ylim = c(-58.5001390148238, 82.99986041915))
...
ggsave(
  ...
  filename = file.path(here::here(), "vignette-outputs", "figures", "slf_binarized_summed_countries_2041-2070_ssp370_GFDL.jpg"),
  height = 8, 
  width = 12,
  device = "jpeg",
  dpi = "retina"
...
ggsave(
  ...
  filename = file.path(here::here(), "vignette-outputs", "figures", "slf_binarized_risk_patchwork.jpg"),
  height = 8, 
  width = 10,
  device = "jpeg",
  dpi = "retina"
...
## Create raw suitability plots for global model
...
global_1995_plot <- ggplot() +
  ...
  scale_fill_gradientn(
    ...
    # labels for the tick marks
    labels = c("unsuitable", "", paste0(as.numeric(summary_global[42, 2]), "\n(MTSS thresh)"), 0.5, 0.75, 1),
    limits = c(0, 1.0),
    guide = guide_colorbar(frame.colour = "black", ticks.colour = "black", barwidth = 20, draw.ulim = TRUE, draw.llim = TRUE)
    ) +
  # labs
...
## Calculate total area predicted by each model
...
# 2055
slf_model_prop_table_2055 <- terra::expanse(
  x = slf_binarized_2055,
  unit = "km",
  byValue = TRUE
) 
...
# join
slf_model_prop_table_joined <- left_join(slf_model_prop_table_1995, slf_model_prop_table_2055, by = c("value", "layer")) %>%
  # add labels
