...
# Create binary threshold maps to infer range filling
...
## 2. Mosaic rasters and produce map
...
*Suitable Agreement = high risk = 10
*Suitable regional/unsuitable global = adaptive risk = 6
*unsuitable regional/suitable global = imminent risk = 9
...
# now, create vectors of values used to manually edit the scale of the plot
# the possible values of the scale and their order
breaks.obj <- unique(slf_binarized_1995_df$global_regional_binarized)
# labels for the values
labels.obj <- c("low risk", "adaptive risk", "high risk", "imminent risk")
# vector of colors to classify scale
values.obj <- c("azure4", "darkblue", "darkred", "darkorange")
...
slf_binarized_1995_plot <- ggplot() +
  ...
  labs(
    title = "Predicted risk of SLF invasion globally",
    subtitle = "1981-2010"
    ) +
  scale_discrete_manual(
    name = "risk for SLF",
...
# now, create vectors of values used to manually edit the scale of the plot
# the possible values of the scale and their order
breaks.obj <- unique(slf_binarized_2055_df$global_regional_binarized)
# labels for the values
labels.obj <- c("low risk", "adaptive risk", "high risk", "imminent risk")
# vector of colors to classify scale
values.obj <- c("azure4", "darkblue", "darkred", "darkorange")
...
slf_binarized_2055_plot <- ggplot() +
  ...
  labs(
    title = "Predicted risk of SLF invasion globally, projected for climate change",
...
In the next vignette, I will create suitability maps to compliment these figures.
