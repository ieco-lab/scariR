...
slf_range_shift_table <- slf_range_shift_table %>%
  dplyr::select(-layer) %>%
  mutate("Ld_range_shift_type" = ranges.obj) %>%
  rename("area_km" = "area") %>%
  dplyr::select(-value) %>%
  relocate(Ld_range_shift_type)


# convert to kable
slf_range_shift_kable <- kable(x = slf_range_shift_table, escape = TRUE) %>%
...
if(FALSE) {
 ...
 # webshot::install_phantomjs()
  ...
  webshot::webshot(
    ...
    file = file.path(here::here(), "vignette-outputs", "figures", "slf_range_shift_summed_area_table.png")
  )
