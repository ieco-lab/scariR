---
title: "Ensemble regional-scale models into a single prediction"
...
```{r Ri.NAmerica regional_invaded model}
...
## 3.1 take ABS value of Exdet rasters
...
```{r weight hist exdet rasters using AUC}
...
```{r weight CMIP6 exdet rasters using AUC- SSP126}
...
```{r weight CMIP6 exdet rasters using AUC- SSP370}
...
```{r weight CMIP6 exdet rasters using AUC- SSP585}
...
## 3.3 Weight models using confidence metric (ExDet)
...
ensemble_thresh_values <- tibble(
  thresh = c(rep("MTP", 4), rep("MTSS", 4)),
  time_period = c("1981-2010", rep("2041-2070", 3), "1981-2010", rep("2041-2070", 3)),
  CMIP6_model = c(NA, "ssp126_GFDL", "ssp370_GFDL", "ssp585_GFDL", NA, "ssp126_GFDL", "ssp370_GFDL", "ssp585_GFDL"),
  value = c(MTP_hist_thresh, MTP_126_thresh, MTP_370_thresh, MTP_585_thresh, MTSS_hist_thresh, MTSS_126_thresh, MTSS_370_thresh, MTSS_585_thresh)
)
...
exdet_regional_ensemble_hist_plot <- exdet_regional_ensemble_hist_plot +
  # new fill scale
...
exdet_126_mean_df <- terra::rast(x =  file.path(
    mypath, "models", "slf_regional_ensemble_v1", "ExDet_slf_regional_ensemble_v1_background_projGlobe_2041-2070_GFDL_ssp126.asc"
    )) %>%
  terra::as.data.frame(xy = TRUE)
...
```{r create CMIP6 exdet plot- ssp 126}
...
# add background area shapes
exdet_regional_ensemble_126_plot <- exdet_regional_ensemble_126_plot +
  ggnewscale::new_scale_fill() +
   # add layer for bg point area
  geom_raster(data = regional_native_background_area_df, aes(x = x, y = y, fill = "Background"), alpha = 0.25) +
  geom_raster(data = regional_invaded_background_area_df, aes(x = x, y = y, fill = "Background"), alpha = 0.25) +
...
# re-load object as df
exdet_370_mean_df <- terra::rast(x =  file.path(
    mypath, "models", "slf_regional_ensemble_v1", "ExDet_slf_regional_ensemble_v1_background_projGlobe_2041-2070_GFDL_ssp370.asc"
    )) %>%
  terra::as.data.frame(xy = TRUE)
...
```{r create CMIP6 exdet plot- ssp 370}
...
# add background area shapes
exdet_regional_ensemble_370_plot <- exdet_regional_ensemble_370_plot +
  ggnewscale::new_scale_fill() +
   # add layer for bg point area
  geom_raster(data = regional_native_background_area_df, aes(x = x, y = y, fill = "Background"), alpha = 0.25) +
  geom_raster(data = regional_invaded_background_area_df, aes(x = x, y = y, fill = "Background"), alpha = 0.25) +
...
# re-load object as df
exdet_585_mean_df <- terra::rast(x =  file.path(
    mypath, "models", "slf_regional_ensemble_v1", "ExDet_slf_regional_ensemble_v1_background_projGlobe_2041-2070_GFDL_ssp585.asc"
    )) %>%
  terra::as.data.frame(xy = TRUE)
...
```{r create CMIP6 exdet plot- ssp 585}
...
# add background area shapes
exdet_regional_ensemble_585_plot <- exdet_regional_ensemble_585_plot +
  # new fill scale
  ggnewscale::new_scale_fill() +
   # add layer for bg point area
  geom_raster(data = regional_native_background_area_df, aes(x = x, y = y, fill = "Background"), alpha = 0.25) +
...
# re-load object as df
exdet_ssp_mean_df <- terra::rast(x =  file.path(
    mypath, "models", "slf_regional_ensemble_v1", "ExDet_slf_regional_ensemble_v1_background_projGlobe_2041-2070_GFDL_ssp_mean.asc"
    )) %>%
  terra::as.data.frame(xy = TRUE)
...
```{r create CMIP6 exdet plot- ssp mean}
...
# add background area shapes
exdet_regional_ensemble_mean_plot <- exdet_regional_ensemble_mean_plot +
  ggnewscale::new_scale_fill() +
   # add layer for bg point area
  geom_raster(data = regional_native_background_area_df, aes(x = x, y = y, fill = "Background"), alpha = 0.25) +
  geom_raster(data = regional_invaded_background_area_df, aes(x = x, y = y, fill = "Background"), alpha = 0.25) +
