...
exdet_regional_ensemble_hist_plot <- ggplot() +
  ...
  breaks = c(
    round(min(values(exdet_hist_mean), na.rm = TRUE), 8), 
    round(0, 1), round(1, 1)#, 
    # no max for this plot
    #round(max(values(exdet_hist_mean), na.rm = TRUE), 8)
    ),
...
exdet_regional_ensemble_CMIP6_plot <- ggplot() +
  ...
  breaks = c(
    round(min(values(exdet_CMIP6_mean), na.rm = TRUE), 8), 
    round(0, 1), round(1, 1), 
    round(max(values(exdet_CMIP6_mean), na.rm = TRUE), 6)
    ),
  guide = guide_colorbar(frame.colour = "black", ticks.colour = "black", barwidth = 20)
  ) +
