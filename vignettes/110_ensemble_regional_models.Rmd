...
relative_weights <- c(
  regional_native = mean(values(exdet_hist_stack[[1]]), na.rm = TRUE),
  regional_invaded = mean(values(exdet_hist_stack[[2]]), na.rm = TRUE),
  regional_invaded_asian = mean(values(exdet_hist_stack[[3]]), na.rm = TRUE)
)
...
ensemble_relative_cont_plot <- ggplot() +
  geom_bar(aes(x = names(relative_weights_prop), y = relative_weights_prop * 100, fill = ensemble_colors), stat = "identity") +
  theme_bw() +
  labs(
