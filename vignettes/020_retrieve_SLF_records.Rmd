...
map_gbif_thinned_Asia <- ggplot() +
  ...
  geom_point(data = gbif_raw, aes(x = longitude, y = latitude), color = "blue", size = 2) +
  geom_point(data = gbif_thinned, aes(x = longitude, y = latitude), color = "darkorange", shape = 2) +
  coord_quickmap(xlim = c(100, 140), ylim = c(20, 50)) +
...
map_lyde_estab <- ggplot() +
  ...
  geom_point(data = lyde_thinned, aes(x = longitude, y = latitude), color = "darkorange", shape = 2) +
  coord_quickmap(xlim = c(-90, -70), ylim = c(35, 45)) +
...
map_published_Asia <- ggplot() +
  ...
  coord_quickmap(xlim = c(100, 140), ylim = c(10, 45)) +
...
1. Bona, S. D., L. Barringer, P. Kurtz, J. Losiewicz, G. R. Parra, and M. R. Helmus. 2023, January 30. lydemapr: an R package to track the spread of the invasive Spotted Lanternfly (Lycorma delicatula, White 1845) (Hemiptera, Fulgoridae) in the United States. bioRxiv. 

2. GBIF Occurrence Download https://doi.org/10.15468/dl.pqxya7 Accessed from R via rgbif (https://github.com/ropensci/rgbif) on 2024-07-29

3. Huron, N. A., J. E. Behm, and M. R. Helmus. 2022. Paninvasion severity assessment of a U.S. grape pest to disrupt the global wine market. Communications Biology 5:655.

4. Lin, Y.-S., and J.-R. Liao. 2024, August 1. Multifaceted Investigation into the Absence and Potential Invasion of Spotted Lanternfly (Lycorma delicatula) in Taiwan. Research Square.
