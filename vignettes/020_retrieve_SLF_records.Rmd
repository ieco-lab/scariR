...
This vignette will provide instructions to retrieve *Lycorma delicatula* (SLF) presence records for the future creation of MaxEnt models. MaxEnt is a presence-only modeling software, and so it does not require recorded absence data to predict the suitable area for SLF. Four categories of data sources will be used in this analysis: [GBIF](https://www.gbif.org/) (Global Biodiversity Information Facility), [lydemapR](https://github.com/ieco-lab/lydemapr), various pieces of peer-reviewed literature, and natural history notes. These data will then be tidied, spatially thinned and compiled into a single `.csv` file of SLF presence records that can be loaded into MaxEnt.
...
I will also perform a few operations specific to rgbif. I will need to enter my user credentials for gbif; to keep these private, I will edit the .Renviron and call them from there. Once this chunk is run and the .Renviron pops up, enter your username, password and email credentials in the following format:
...
I will also retrieve a list of country abbreviations to tailor the download. I will be excluding countries in North America (USA, Mexico, Canada).
...
occ_download_wait('0041651-240626123714530')
...
# unzip
utils::unzip(
  zipfile = file.path(here::here(), "data-raw", "0041651-240626123714530.zip"),
  exdir =  file.path(here::here(), "data-raw")
  )
# rename .csv file
file.rename(
  from = file.path(here::here(), "data-raw", "0041651-240626123714530.csv"),
  to = file.path(here::here(), "data-raw", paste0("slf_gbif_raw_query_", format(Sys.Date(), "%Y-%m-%d"), ".csv"))
  )
# delete original zip
file.remove(file.path(here::here(), "data-raw", "0041651-240626123714530.zip"))
...
slf_gbif_download_simple <- slf_gbif_download %>%
  mutate(prov = "gbif") %>%
  dplyr::select(c("species", "decimalLongitude", "decimalLatitude", "prov", "year", "gbifID")) %>% # select desired columns
...
```{r retrieve data citation}

gbif_citation('0041651-240626123714530')

```

Here is the data citation: "GBIF Occurrence Download https://doi.org/10.15468/dl.pqxya7 Accessed from R via rgbif (https://github.com/ropensci/rgbif) on 2024-07-29"
...
# read in data
slf_gbif_coords1 <- read_csv(file = file.path(here::here(), "data-raw", "slf_gbif_raw_coords_2024-07-29.csv"))
...
slf_gbif_coords1 <- slf_gbif_coords1 %>%
  dplyr::filter(key != "2860187641") %>% #rm OR---lat, lon:(43.63691, -121.85569)
  dplyr::filter(key != "2862292948") %>% #rm NE---lat, lon:(42.50641,-101.01562)
  dplyr::filter(key != "2864687343") %>% #rm DE---lat, lon:(37.91855, -75.14999)
  dplyr::filter(!key %in% c("2856537682", "2851117559")) #rm MA---lat, lon:(42.20994, -71.18331)
...
Spatial thinning left 505 of 1,489 observations. Finally, we will de-duplicate the data.
...
write_csv(slf_gbif_coords3, file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_2024-07-29.csv"))

# also save as a .rds file
write_rds(slf_gbif_coords3, file = file.path(here::here(), "data", "slf_gbif_cleaned_coords_2024-07-29.rds"))
...
if (TRUE) {
  ...
  slf_lyde_raw <- read_csv(file = file.path(here::here(), "data-raw", "slf_lyde_raw_coords_2024-07-29.csv"))
...
# read in data
slf_lyde <- read_csv(file = file.path(here::here(), "data-raw", "slf_lyde_raw_coords_2024-07-29.csv"))
...
Spatial thinning for the `lyde` dataset left 364 points out of the 39,000 points that fit our needs.
...
write_csv(slf_lyde3, file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_2024-07-29.csv"))

# also save as a .rds file
write_rds(slf_lyde3, file = file.path(here::here(), "data", "slf_lyde_cleaned_coords_2024-07-29.rds"))
...
# raw data
gbif_raw <- read_csv(file = file.path(here::here(), "data-raw", "slf_gbif_raw_coords_2024-07-29.csv"))

# thinned data
gbif_thinned <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_2024-07-29.csv"))
...
# raw data
lyde_raw <- read_csv(file = file.path(here::here(), "data-raw", "slf_lyde_raw_coords_2024-07-29.csv"))

# thinned data
lyde_thinned <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_2024-07-29.csv"))
...
# patchwork display of plots
 map_lyde_allRecords + map_lyde_noreg +
  plot_layout(ncol = 2)
...
slf_published_papers <- read_csv(file.path(here::here(), "data-raw", "slf_publishedOccurrenceRecords_papers.csv"))
...
slf_published <- read_csv(file.path(here::here(), "data-raw", "slf_publishedOccurrenceRecords_v2.csv"))
...
slf_gbif_coords3 <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_2024-07-29.csv"))

slf_lyde3 <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_2024-07-29.csv"))
...
write_csv(x = slf_all_coords, file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_2024-07-29.csv"))
...
slf_all_coords <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_2024-07-29.csv"))
...
if(TRUE) {
  ...
  slf_all_coords2 <- spThin::thin(
    ...
    write.log.file = TRUE, 
    log.file = file.path(here::here(), "vignette-outputs", "data-tables", paste0("slf_all_coords_cleaned_thinning_log_2024-07-29.txt")),
    ...
    out.base = paste0("slf_all_coords_cleaned_2024-07-29"),
...
It seems from plot 3 that 813 records were retained during more of the reps. However, this is not very different from the maximum, so we will keep the maximum of 814.

Now, lets load in the final thinned dataset for a final round of cleaning.

```{r read in thinned data}

slf_all_coords2 <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_cleaned_2024-07-29_thin1.csv"))
...
# save
write_csv(x = slf_all_coords3, file = file.path(here::here(), "vignette-outputs", "data-tables", paste0("slf_all_final_coords_2024-07-29.csv")))

# also save as a .rds file
write_rds(slf_all_coords3, file = file.path(here::here(), "data", paste0("slf_all_final_coords_2024-07-29.rds")))

```

```{r map all points}

slf_all_coords3 <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_final_2024-07-29.csv"))
...
```{r number points invaded vs native range}
...
if(FALSE) {
  ...
  # new data
  slf_new <- read_csv(file = file.path(here::here(), "data-raw", "data-old", "slf_gbif_cleaned_2024-07-29.csv"))
  
  # old data
  slf_old <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_2024-01-05.csv"))
...
3. GBIF Occurrence Download https://doi.org/10.15468/dl.pqxya7 Accessed from R via rgbif (https://github.com/ropensci/rgbif) on 2024-07-29
