---
title: "Retrieve SLF records from various sources and tidy them for MaxEnt modeling (chapter 1)"
author: "Sam Owens"
date: "2024-01-05"
...
* 2024-01-05- adapted to use `rgbif` instead of `spocc` for gbif download
...
I will also perform a few operations specific to rgbif. I will need to enter my user credentials for gbif; to keep these private, I will edit the .Renviron and call them from there. I will also retrieve a list of country abbreviations to tailor the download. I will be excluding countries in North America (USA, Mexico, Canada).

```{r other rgbif operations}

# edit R environment for user credentials
# enter user credentials using the following: GBIF_USER; GBIF_PWD; GBIF_EMAIL
...
occ_download_wait('0046146-231120084113126')
...
slf_gbif_download <- occ_download_get(
  key = '0046146-231120084113126',
  path = file.path(here::here(), "data-raw"),
  # overwrite = TRUE
  ) %>% 
  occ_download_import()

# retrieve data citation
gbif_citation('0046146-231120084113126')
 
# rename .zip file
file.rename(
  from = file.path(here::here(), "data-raw", "0046146-231120084113126.zip"),
  to = file.path(here::here(), "data-raw", paste0("slf_gbif_raw_query_", format(Sys.Date(), "%Y-%m-%d"), ".zip"))
  )

# simplify download into more useable .csv
slf_gbif_download_simple <- slf_gbif_download %>%
  mutate(prov = "gbif") %>%
  dplyr::select(c("species", "decimalLongitude", "decimalLatitude", "prov", "eventDate", "gbifID")) %>% # select desired columns
...
slf_gbif_download_simple$date <- as_date(slf_gbif_download_simple$date)
...
# read in data
slf_gbif_coords1 <- read_csv(file = file.path(here::here(), "data-raw", "slf_gbif_raw_coords_2024-01-05.csv"))
...
Spatial thinning left 5888 of 17,339 observations, so it is apparent that the spatial thinning was needed. On the run without records in N. America, it left 433 of 1,301 observations. Finally, we will de-duplicate the data.
...
write_csv(slf_gbif_coords3, file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_2024-01-05.csv"))

# also save as a .rds file
write_rds(slf_gbif_coords3, file = file.path(here::here(), "data", "slf_gbif_cleaned_coords_2024-01-05.rds"))
...
The next chunk is an import from an unpublished version of the lydemapr dataset called "tinyslf". This version will be published at on the package soon. This file was named "slf_lyde_raw_coords_2023-12-12.csv" for consistency with previous file naming. It contains about 900,000 published SLF records and includes the years 2022 and 2023.

```{r load in and tidy unpublished version of lydemapr dataset}

if (FALSE) {
  
  slf_lyde_raw <- read_csv(file = file.path(here::here(), "data-raw", "slf_lyde_raw_coords_2023-12-12.csv"))

  # tidy
  slf_lyde_raw %<>%
    dplyr::select(-survey) %>%
    rename(
      "lyde_present" = "slf_present",
      "lyde_established" = "slf_established",
      "lyde_density" = "slf_density"
    )
  
  # overwrite save original version
  write_csv(x = slf_lyde_raw, file = file.path(here::here(), "data-raw", "slf_lyde_raw_coords_2023-12-12.csv"))
...
# read in data
slf_lyde <- read_csv(file = file.path(here::here(), "data-raw", "slf_lyde_raw_coords_2023-12-12.csv"))
...
write_csv(slf_lyde3, file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_2023-12-12.csv"))

# also save as a .rds file
write_rds(slf_lyde3, file = file.path(here::here(), "data", "slf_lyde_cleaned_coords_2023-12-12.rds"))
...
# raw data
gbif_raw <- read_csv(file = file.path(here::here(), "data-raw", "slf_gbif_raw_coords_2024-01-05.csv"))

# thinned data
gbif_thinned <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_2024-01-05.csv"))
...
# raw data
lyde_raw <- read_csv(file = file.path(here::here(), "data-raw", "slf_lyde_raw_coords_2023-12-12.csv"))

# thinned data
lyde_thinned <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_2023-12-12.csv"))
...
slf_gbif_coords3 <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_2024-01-05.csv"))

slf_lyde3 <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_2023-12-12.csv"))
...
write_csv(x = slf_all_coords, file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_2024-02-01.csv"))
...
slf_all_coords <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_2024-02-01.csv"))
...
if(TRUE) {
  ...
  slf_all_coords2 <- spThin::thin(loc.data = slf_all_coords, 
                              ...
                              write.log.file = TRUE, 
                              log.file = file.path(here::here(), "vignette-outputs", "data-tables", paste0("slf_all_coords_cleaned_thinning_log_2024-02-01.txt")),
                              ...
                              out.base = paste0("slf_all_coords_cleaned_2024-02-01"),
...
slf_all_coords2 <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_cleaned_2024-02-01_thin1.csv"))
...
# save
write_csv(x = slf_all_coords3, file = file.path(here::here(), "vignette-outputs", "data-tables", paste0("slf_all_final_coords_2024-02-01.csv")))

# also save as a .rds file
write_rds(slf_all_coords3, file = file.path(here::here(), "data", paste0("slf_all_final_coords_2024-02-01.rds")))

```

```{r map all points}

slf_all_coords3 <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_final_coords_2024-02-01.csv"))
...
# save output
ggsave(SLF_by_longitude, filename = file.path(here::here(), "vignette-outputs", "figures", "SLF_final_all_coords_by_longitude.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
if(FALSE) {
  ...
  slf_coords4 <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_2024-01-05.csv"))
  ...
  map_errors <- ggplot() +
    geom_polygon(data = map_data('world'), aes(x = long, y = lat, group = group), fill = NA, color = "black", lwd = 0.15) +
    geom_point(data = slf_error_points, aes(x = x, y = y), color = "darkorange", size = 2) +
    coord_quickmap(xlim = c(-164.5, 163.5), ylim = c(-55, 85)) +
    ggtitle("SLF errors") +
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank(),
  ...
  write_csv(slf_coords4, file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_2024-01-05_v1.csv"))
  
  # also save as a .rds file
  write_rds(slf_coords4, file = file.path(here::here(), "data", "slf_gbif_cleaned_2024-01-05_v1.rds"))
...
if(FALSE) {
  ...
  # old data
  slf_2020 <- read_csv(file = file.path(here::here(), "data-raw", "data-old", "slf_gbif_cleaned_coords_2022.csv"))
  
  # new data
  slf_new <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_2024-01-05.csv"))
  
  # plot SLF
  map_slf <- ggplot() +
    geom_polygon(data = map_data('world'), aes(x = long, y = lat, group = group), fill = NA, color = "black", lwd = 0.15) +
    geom_point(data = slf_2020, aes(x = longitude, y = latitude), color = "darkorange", size = 2) +
...
3. GBIF Occurrence Download https://doi.org/10.15468/dl.8mz3km Accessed from R via rgbif (https://github.com/ropensci/rgbif) on 2024-01-05
