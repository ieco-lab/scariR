...
ggsave(slf_suitability_scatter, filename = file.path(here(), "vignette-outputs", "figures", "slf_suitability_maxent_v5_plot.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
(IVR_suitability_scatter <- ggplot() +
  ...
  scale_x_continuous(name = "suitability in global model", limits = c(0, 1), breaks = breaks) + # same break scale as for the SLF data
  scale_y_continuous(name = "suitability in regional model", limits = c(0, 1), breaks = breaks) +
  theme_minimal()
)
...
ggsave(IVR_suitability_scatter, filename = file.path(here(), "vignette-outputs", "figures", "IVR_suitability_maxent_v5_plot.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
Now I will plot the IVRs (purple) and known SLF points (orange) over the v5 version of the global model I produced.

```{r plot SLF suitability values}

# plot with points
v5_global_IVR_SF <- ggplot() +
  geom_raster(data = terra::as.data.frame(v5_global1, xy = TRUE), 
              aes(x = x, y = y, fill = v5_global)) +
  geom_point(data = slf_points, 
             aes(x = x, y = y), color = "darkorange", size = 0.1) +
  geom_point(data = IVR_points, 
             aes(x = x, y = y), color = "darkviolet", size = 0.1) +
  xlab("longitude") +
  ylab("latitude") +
  ...
  labs(title = "Contemporary slf suitability in global model") +
  coord_equal() 
...
ggsave(v5_global_IVR_SLF, filename = file.path(here(), "vignette-outputs", "figures", "IVR_suitability_maxent_v5_plot.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
...
# first, convert -9999 values to 1 (unsuitable)
v5_regional2_df$v5_regional[v5_regional2_df$v5_regional == -9999] <- 1
...
ggsave(v5_summed_points_plot, filename = file.path(here(), "vignette-outputs", "figures", "v5_maxent_overlay_slfPoints.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
