...
# slf presences
p_slf_points <- read_rds(file = file.path(here::here(), "data", "slf_all_final_coords_2024-07-29.rds")) %>%
  dplyr::select(-species) %>%
  as.data.frame()

# entire eastern USA background point set
a_global_background_points <- read_rds(file = file.path(here::here(), "data", "global_background_points_v3.rds")) %>%
  as.data.frame()
...
I usually look at how many records were dropped before I save it, because SDMtune gives an undefined warning about how many samples were discarded. In this case, we only lost 5 presence records, so this is acceptable. I will also save the output to be used later.
...
## 2.2 Re-training based on tuning
