...
First, I will load in the SLF presence dataset created in vignette 030 and the background points dataset created in the last vignette.
...
Next, I will load in and stack the covariates needed for the model.
...
I will load in 3 sets of CMIP6 covariates, 1 per ssp scenario.
...
readr::write_rds(x = global_test, file = file.path(mypath, "global_test.rds"))
...
I will use these hyperparameters to train the initial model, but these parameters may change based on model tuning, which I will perform next. 20% of the held apart data will be used for the tuning the hyperparameters (val), while the other 20% will be used for testing the models.
...
Based on the test AUC metric, we see that the highest value was attributed to the model with a regularization multiplier of 1.5 and qpht features (model 11). I will also check based on the TSS.
...
```
However, based on the test TSS metric, we see that the highest value was attributed to the model with a regularization multiplier of 1.5 and lqpht (ALL) features (model 12). 

We will need to choose either model 11 or model 12. Since the difference in test AUC and TSS are incredibly small between both models, I will choose model 12 because this model has been consistently more fit across model versions. These tuned parameters will also be applied to all future models performed for SLF at the regional scale. 
...
# save tuned models
write_rds(global_model_tuned_auc, file = file.path(mypath, "global_model_tuned_auc.rds"))
write_rds(global_model_tuned_tss, file = file.path(mypath, "global_model_tuned_tss.rds"))
...
I will use the `SDMtune::predict()` function to predict the suitability for the range of interest. I will threshold the map by the `MTP`, `MTSS` and `10_percentile` thresholds. The workflow for this application can be found in vignette `052_create_suitability_maps_workflow.Rmd` and is wrapped into the function `slfSpread::create_MaxEnt_suitability_maps_CV()`.
...
I will get projected suitability values, calculated on the cloglog scale (between 0 and 1) for each of the known SLF populations. These suitability values will be added back to the original data frame and saved. The workflow for this application can be found in vignette `053_predict_xy_suitability_workflow.Rmd` and is wrapped into the function `slfSpread::predict_xy_suitability_CV()`. This function extracts covariate data from each data point and then uses the model object to predict the suitability for SLF establishment at that location based on the climate variables. This function has two modes for predicting the suitability: simple and buffered. I will use simple (the suitability at that exact point) for the location of SLF populations. 
...
# write to rds for use downstream

summary_global <- read_csv(file.path(mypath, "global_summary_all_iterations.csv"))

readr::write_rds(summary_global, file = file.path(here::here(), "data", "global_threshold_values.rds"))
