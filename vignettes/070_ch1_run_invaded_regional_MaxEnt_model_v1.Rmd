...
rm(x_easternUSA_env_covariates_list)
...
a_regional_invaded_background_points <- read.csv(file.path(here(), "vignette-outputs", "data-tables", "regional_invaded_background_points_v1.csv"))

```

I will need to combine the above covariate, presence and background point datasets into two different datasets to feed into MaxEnt: one for training and another for testing the model. These dataset will need to contain point-wise values for each of the predictor covariates at the presence and background point coordinates. `SDMtune` takes an SWD (sample with data) object for these purposes, containing the presences and background points with associated covariate data to be fed into the model. First, I will create the training dataset.

### Model training object

I will need to perform some extra edits on the regional version of the slf presences. I will need to filter these presences by two different extents: the invaded range and the native range. I will filter the presences from the invaded range and will use these points to train the invaded model.
...
SDMtune::swd2csv(swd = regional_invaded_train, file_name = c(
  file.path(here(), "vignette-outputs", "data-tables", "regional_invaded_train_slf_presences_with_data.csv"),
...
SDMtune::swd2csv(swd = regional_invaded_test, file_name = c(
  file.path(here(), "vignette-outputs", "data-tables", "regional_invaded_test_slf_presences_with_data.csv")
  ))
...
First, I will train a model with 10 cross-validated replicates. The model will only be trained on the proportion of presence data from the invaded range. It will then be validated using the presence data from the native range (China).
...
if (TRUE) {
  ...
  getTunableArgs(regional_invaded_model)
  ...
  regional_invaded_model_tuned <- SDMtune::gridSearch(
  ...
  regional_invaded_model_tuned@results
  # choose model with highest test AUC
  best.model <- which.max(regional_invaded_model_tuned@results$test_AUC)
  
}

```

Based on the model tuning, we see that the highest test AUC value was attributed to the model with a regularization multiplier of 0.25 and hinge features (model 1). We will need to re-train the model using these parameters.
...
mypath <- file.path(here() %>% 
                       dirname(),
                     "maxent/models/slf_regional_invaded_v1")
...
# save model tuning object
write_rds(regional_invaded_model_tuned, file = file.path(mypath, "regional_invaded_model_tuned.rds"))
