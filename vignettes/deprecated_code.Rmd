...
I subtracted 0.1 from each 1995 point and added 0.1 to each 2055 point so the arrows would be more visible
...
regional_ensemble_MTSS_2055 <- as.numeric(xy_regional_ensemble_1995_rescaled_thresholds[4, 2])
 ...
 quad_counts <- tibble(
   ...
   extreme_risk_quad4_1995 = c(
     xy_joined_rescaled %>% dplyr::select(xy_global_1995_rescaled, regional_ensemble_MTSS_1995) %>% dplyr::filter(xy_global_1995_rescaled >= global_MTSS & xy_regional_ensemble_1995_rescaled >= regional_ensemble_MTSS_1995) %>% dplyr::select(xy_global_1995_rescaled) %>% colMeans() - 0.1,
     xy_joined_rescaled %>% dplyr::select(xy_regional_ensemble_1995_rescaled) %>% dplyr::filter(xy_regional_ensemble_1995_rescaled >= regional_ensemble_MTSS_1995) %>% colMeans() - 0.1
   ),
   # top left quadrant
   high_risk_quad3_1995 = c(
     xy_joined_rescaled %>% dplyr::select(xy_global_1995_rescaled) %>% dplyr::filter(xy_global_1995_rescaled < global_MTSS) %>% colMeans() - 0.1,
     xy_joined_rescaled %>% dplyr::select(xy_regional_ensemble_1995_rescaled) %>% dplyr::filter(xy_regional_ensemble_1995_rescaled >= regional_ensemble_MTSS_1995) %>% colMeans() - 0.1
   ),
   ...
   extreme_risk_quad4_2055 = c(
     xy_joined_rescaled %>% dplyr::select(xy_global_2055_rescaled) %>% dplyr::filter(xy_global_2055_rescaled >= global_MTSS) %>% colMeans() + 0.1,
     xy_joined_rescaled %>% dplyr::select(xy_regional_ensemble_2055_rescaled) %>% dplyr::filter(xy_regional_ensemble_2055_rescaled >= regional_ensemble_MTSS_2055) %>% colMeans() + 0.1
   ),
   # top left quadrant
   ...
   dplyr::relocate(axis, model, contains("extreme"), contains("high"), contains("moderate"))

# this code was inspired by Smith 2021

```

```{r plot SLF suitability values with mean trendlines, fig.asp = 1}
...
# plot
(xy_joined_rescaled_plot <- ggplot() +
   # threshold lines
   # MTSS thresholds
   ...
   # these arrows will only include the mean of each quadrant
   # quad4, extreme suit, top right
   geom_segment(
      aes(
        x = as.numeric(quad_counts[1, 3]),
        xend = as.numeric(quad_counts[1, 4]),
        y = as.numeric(quad_counts[2, 3]),
        yend = as.numeric(quad_counts[2, 4])
      ), 
     arrow = grid::arrow(angle = 25, type = "closed"), linewidth = 2, color = "darkred"
   ) +
    # quad3, high suitability, top left
   geom_segment(
     aes(
       x = as.numeric(quad_counts[1, 5]),
       xend = as.numeric(quad_counts[1, 6]),
       y = as.numeric(quad_counts[2, 5]),
       yend = as.numeric(quad_counts[2, 6])
     ), 
     arrow = grid::arrow(angle = 25, type = "closed"), linewidth = 2, color = "darkorange"
   ) +
   # quad2, moderate suitability, bottom right
   geom_segment(
     aes(
       x = as.numeric(quad_counts[1, 7]),
       xend = as.numeric(quad_counts[1, 8]),
       y = as.numeric(quad_counts[2, 7]),
       yend = as.numeric(quad_counts[2, 8])
     ), 
     arrow = grid::arrow(angle = 25, type = "closed"), linewidth = 2, color = "gold"
   ) +
   # quad1, low suitability, bottom left
   geom_segment(
     aes(
       x = as.numeric(quad_counts[1, 9]),
       xend = as.numeric(quad_counts[1, 10]),
       y = as.numeric(quad_counts[2, 9]),
       yend = as.numeric(quad_counts[2, 10])
     ), 
     arrow = grid::arrow(angle = 25, type = "closed"), linewidth = 2, color = "azure4"
   ) +
   # axes scaling
    # axes scaling
   ...
   geom_label(aes(x = 0.75, y = 1, hjust = "right", vjust = "top", label = "extreme risk"), fill = "darkred", color = "azure", size = 5) +
   # high risk, top left, quad3
   geom_label(aes(x = 0.3, y = 1, hjust = "left", vjust = "top", label = "high risk"), fill = "darkorange", color = "azure", size = 5) +
   # moderate risk, bottom right, quad2
   geom_label(aes(x = 0.75, y = 0, hjust = "right", vjust = "bottom", label = "moderate risk"), fill = "gold", color = "azure", size = 5) +
   # low risk, bottom left, quad1
   geom_label(aes(x = 0.3, y = 0, hjust = "left", vjust = "bottom", label = "low risk"), fill = "azure4", color = "azure", size = 5) +
   # aesthetics
