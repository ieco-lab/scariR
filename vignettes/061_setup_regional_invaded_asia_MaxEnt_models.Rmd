...
regional_invaded_asian_points <- read.csv(file = file.path(here(), "vignette-outputs", "data-tables", "regional_invaded_asian_background_points_v0.csv"))
...
ggsave(regional_invaded_asian_points_plot, 
       filename = file.path(here(), "vignette-outputs", "figures", "regional_invaded_asian_model_background_points_v0.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina"
...
regional_invaded_asian_slf_plot <- ggplot() +
  geom_raster(data = global_mean_df, aes(x = x, y = y), fill = "azure4") +
  geom_raster(data = invaded_asian_df, aes(x = x, y = y), fill = "azure1") +
  geom_point(data = slf_points, aes(x = x, y = y), color = "darkorange", size = 0.25) +
  labs(
