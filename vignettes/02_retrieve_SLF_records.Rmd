...
The returned file is an `occdat` file, which has an element for all seven databases that the `spocc` package has access to. We want to isolate the raw data request from only `GBIF` and then save that as a `.csv` file. We also want to convert this data object into a data frame and save that as a separate `.csv`.
...
if(FALSE) {
  ...
  write_csv(x = slf_gbif_final, 
            file = file.path(here(), "data-raw", paste0("slf_gbif_", format(Sys.Date(), "%Y-%m-%d"), ".csv")))
  ...
  write_csv(x = slf_gbif_coords1, 
            file = file.path(
              here(), "data-raw", 
              paste0("slf_gbif_raw_coords_", format(Sys.Date(), "%Y-%m-%d"), ".csv"))
            )
...
The raw pull from GBIF brought in 17,339 total records of SLF. The raw query and the raw coordinates were saved to the `data-raw` folder. I will read in the coordinate data that was just saved in preparation for data tidying. 

```{r read in raw data}

slf_gbif_coords1 <- read_csv(file = file.path(here(), "data-raw", "slf_gbif_raw_coords_2023-07-12.csv"))
...
In `humboldt::humboldt.occ.rarefy`, The data will be rarefied to 1km over 10 passes. This will be done once more on the final dataset. Spatial thinning should reduce autocorrelation and sampling bias. I have programmed this function to rarefy at 1km, which is the resolution of the climate data we will use from `CHELSA` (30 arc-seconds, roughly 1km at the equator). This way, no more than one occurrence will be left per grid cell.
...
Spatial thinning left 5888 of 17,339 observations, so it is apparent that the spatial thinning was needed. Finally, we will de-duplicate the data.
...
write_csv(slf_gbif_coords3, file = file.path(here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_v2.csv"))

# also save as a .rda file
save(slf_gbif_coords3, file = file.path(here(), "data", "slf_gbif_cleaned_coords_v2.rda"))
...
# read in data
slf_lyde <- read_csv(file = file.path(here(), "data-raw", "slf_lyde_raw_coords_2023-07-13.csv"))
...
write_csv(slf_lyde3, file = file.path(here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_v1.csv"))

# also save as a .rda file
save(slf_lyde3, file = file.path(here(), "data", "slf_lyde_cleaned_coords_v1.rda"))
...
# raw data
gbif_raw <- read_csv(file = file.path(here(), "data-raw", "slf_gbif_raw_coords_2023-07-12.csv"))

# thinned data
gbif_thinned <- read_csv(file = file.path(here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_v2.csv"))
...
# thinned data
lyde_thinned <- read_csv(file = file.path(here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_v1.csv"))
...
slf_gbif_coords3 <- read_csv(file.path(here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_coords_v2.csv"))

slf_lyde3 <- read_csv(file.path(here(), "vignette-outputs", "data-tables", "slf_lyde_cleaned_coords_v1.csv"))
...
# tidy gbif data
slf_gbif_coords3 %<>%
  select(name:latitude, key, prov) %>%
  rename("data_source" = "prov",
         "species" = "name")

# tidy lyde data
slf_lyde3 %<>%
  select(species, longitude, latitude, pointID) %>%
  rename("key" = "pointID") %>%
  mutate(data_source = "lyde")
  
# published data
slf_published %<>%
  select(name:key, publishingArticle) %>%
  rename("species" = "name",
...
write_csv(x = slf_all_coords, file = file.path(here(), "vignette-outputs", "data-tables", "slf_all_coords.csv"))
...
slf_all_coords <- read_csv(file.path(here(), "vignette-outputs", "data-tables", "slf_all_coords.csv"))
...
slf_all_coords2 <- read_csv(file.path(here(), "vignette-outputs", "data-tables", "slf_all_coords_cleaned_2023-07-14_thin1.csv"))
...
# save
write_csv(x = slf_all_coords3, file = file.path(here(), "vignette-outputs", "data-tables", paste0("slf_all_final_coords_v0_", format(Sys.Date(), "%Y-%m"), ".csv")))

# also save as a .rda file
save(slf_all_coords3, file = file.path(here(), "data", paste0("slf_all_final_coords_v0_", format(Sys.Date(), "%Y-%m"), ".rda")))
...
if(FALSE) {
  ...
  slf_coords4 <- read_csv(file = file.path(here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_v2.csv"))
  ...
  write_csv(slf_coords4, file = file.path(here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_v3.csv"))
  
  # also save as a .rda file
  save(slf_coords4, file = file.path(here(), "data", "slf_gbif_cleaned_v3.rda"))
...
if(FALSE) {
  ...
  # new data
  slf_new <- read_csv(file = file.path(here(), "vignette-outputs", "data-tables", "slf_gbif_cleaned_v2.csv"))
