...
To do that, I will use the `terra::app` function to convert the global mean raster to a list of suitability values. Then I will use the `dismo::randomPoints()` function again, but will give it the raster of weighted suitability values so that it chooses values according to the weight. 
...
global_mean_weighted <- raster::raster(x = file.path(mypath, "global_predicted_suitability_NAmerica_1981-2010_projected_mean_weighted.asc")) %>%
  raster::crop(x = ., y = ext.obj)
...
(regional_points_plot <- ggplot() +
    ...
    geom_point(data = regional_points, aes(x = x, y = y), color = "darkorange", size = 0.25) +
...
We can see that the background points were selected from areas where the model did not predict high suitability (most highly suitable areas were near PA and NJ). This figure is roughly opposite of the mean global output figure. 


```{r save plot}

ggsave(regional_points_plot, 
       filename = file.path(here(), "vignette-outputs", "figures", "regional_background_points.jpg"),
       height = 8, 
       width = 10,
       device = "jpeg",
       dpi = "retina")
